...
You receive a Jira issue key or URL (e.g. `SVY-21234`).
...
curl -s -H "Authorization: Basic $token" "https://servoy-cloud.atlassian.net/rest/api/3/issue/SVY-21234"
```
...
Parse the input to get the bare issue key (e.g. `SVY-21234`).
...
| Non-functional requirements | Performance, bundle size, backward compatibility? |
...
- Identify components, services, and patterns to follow
- Check which Angular module the affected code belongs to
...
Example: `../../docs/SVY-21234-fix-signal-write-upload.spec.md`
...
## 2. Background
<Relevant existing behaviour, architecture context, prior art. Reference the
Angular layer (sablo, ngclient, servoycore, designer) where the change lives.>

## 3. Design

### 3.1 <First design area>
<Describe the proposed design. Reference Angular patterns (signals, computed,
services, components) as applicable.>

### 3.2 <Second design area>
...

## 4. Implementation plan
<Ordered list of the concrete changes needed — files to create/modify, signals
to introduce, template changes, etc. This becomes the coding agent's task list.>
...
```
../../docs/SVY-21234-fix-signal-write-upload.spec.md
```
