...
import { CUSTOM_ELEMENTS_SCHEMA, Component, TemplateRef, ChangeDetectionStrategy, Renderer2, ChangeDetectorRef, DebugElement, input, output, viewChild, signal } from '@angular/core';

import { FormattingService, TooltipService, LoggerFactory, ServoyBaseComponent, ICustomObjectValue, ICustomArrayValue, ServoyPublicModule, WindowRefService, SpecTypesService } from '@servoy/public';
...
@Component({
  template: ` <svy-form [name]="'aForm'" [injectedComponentRefs]="getCustomTestComponentTemplates()"></svy-form>
    <ng-template #customTestComponent let-callback="callback" let-state="state">
      @if (state.model.visible) {
        <testcomponents-custom-component
          ...
          [arrayOfCustomObjects]="state.model.arrayOfCustomObjects"
          (arrayOfCustomObjectsChange)="callback.datachange(state, 'arrayOfCustomObjects', $event)"
...
interface ComponentModelContents {
  divLocation: number;
  arrayOfCustomObjects: ICustomArrayValue<SomeCustomObject>;
...
describe.skip('FormComponentComponentTest', () => {
  ...
  beforeEach(async () => {
    ...
    TestBed.configureTestingModule({
      imports: [FormComponent, TestComponentsCustomComponent, AddAttributeDirective, ServoyCoreSlider, ErrorBean, TestHostComponent, ServoyTestingModule, ServoyPublicModule],
      providers: [
  ...
  it('adding an element to the shallow-element array should send stuff to server right away; then altering that should send it again as well', () => {
    ...
    expect(sabloService.callService).toHaveBeenCalledWith(
      ...
      'dataPush',
      {
        ...
        beanname: 'myCustomTestComponent',
        changes: {
          arrayOfCustomObjects: { vEr: 0, v: [{ vEr: 0, v: { active: true } }] },
        },
      },
      true,
    );
