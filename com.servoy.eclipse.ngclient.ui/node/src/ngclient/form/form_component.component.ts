...
@Component({
  ...
  template: `
    @if (formCache.absolute) {
      <div [ngStyle]="getAbsoluteFormStyle()" class="svy-form" [ngClass]="formClasses()" svyAutosave>
        <!-- main div -->
  ...
  `,
  standalone: true,
  imports: [CommonModule, FormsModule, AutosaveDirective, AllComponentsModule, AllServicesModules, ...SERVOYCORE_COMPONENTS, ListFormComponent],
...
export class FormComponent extends AbstractFormComponent implements OnDestroy, OnChanges, AfterViewInit, AfterViewChecked, IFormComponent {
  ...
  public static doCallApiOnComponent(
    comp: ServoyBaseComponent<any>,
  ...
  ngOnChanges(changes: SimpleChanges) {
    ...
    if (changes.name) {
      ...
      if (styleClasses) this.formClasses.set(styleClasses.split(' '));
      else this.formClasses.set(null);
      ...
      if (instanceOfApiExecutor(comp)) {
        ...
        this.log.error(
          'trying to call api: ' +
            ...
            path +
            ', but comp: ' +
            (comp == null ? ' is not found' : comp.name + ' doesnt implement IApiExecutor'),
        );
      }
      return null;
    } else {
      return FormComponent.doCallApiOnComponent(
        this.componentCache[componentName],
        this.formCache.getComponentSpecification(componentName)!,
        apiName,
        args,
