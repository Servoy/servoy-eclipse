import { Component, Input, TemplateRef, Renderer2, ChangeDetectionStrategy, SimpleChange } from '@angular/core';
...
export abstract class AbstractFormComponent {
  ...
  public static doCallApiOnComponent(
    ...
    (args as any[])?.forEach(
      (val: any, i: number) =>
        (args[i] = converterService.convertFromServerToClient(
          val,
          callSpec?.getArgumentType(i),
          undefined!,
          undefined!,
          undefined!,
    ...
    if (comp) {
      ...
      if (proto[apiName]) {
        return Promise.resolve(proto[apiName].apply(comp, args)).then(
