...
export class FormService {
      ...
      .then((session) => {
        session.onMessageObject((msg: any) => {
          ...
          if (msg.call) {
            ...
            if (this.log.logLevel >= LogLevel.SPAM)
              this.log.spam(
                this.log.buildMessage(
                  () =>
                    '[compAPIcalled] Received API call from server: "' +
                    componentCall.api +
                    '" to form ' +
                    componentCall.form +
                    ', component ' +
                    (componentCall.propertyPath ? componentCall.propertyPath.toString() : componentCall.bean),
                ),
              );
            ...
            const callItOnceClientFunctionsAreLoaded = () => {
              ...
              if (this.log.logLevel >= LogLevel.SPAM)
                this.log.spam(
                  this.log.buildMessage(
                    () =>
                      '[compAPIcalled] in "callItOnceClientFunctionsAreLoaded" 1 for API call from server: "' +
                      componentCall.api +
                      '" to form ' +
                      componentCall.form +
                      ', component ' +
              ...
              this.clientFunctionService.waitForLoading().finally(() => {
                // this can execute sync right away if there is no need to wait for client functions to load
                ...
                if (this.formComponentCache.has(componentCall.form) && !(this.formComponentCache.get(componentCall.form) instanceof Deferred)) {
                  if (this.log.logLevel >= LogLevel.SPAM)
                    this.log.spam(
                      this.log.buildMessage(
                        () =>
                          '[compAPIcalled] in "callItOnceClientFunctionsAreLoaded" 2; really calling it now; for API call from server: "' +
                          componentCall.api +
                          '" to form ' +
                          componentCall.form +
                          ', component ' +
                  ...
                  this.log.error(
                    this.log.buildMessage(
                      () =>
                        '[compAPIcalled] calling api ' +
                        componentCall.api +
                        ' in form ' +
                        componentCall.form +
                        ' on component ' +
                        componentCall.bean +
                        '. Form was loaded but it got unloaded while waiting for clientFunctionServices to load; unexpected... Api call was skipped.',
                    ),
                  );
            ...
            const waitForFormToShowOnClientThenCallAPI = (timeOutIfItDoesNotShowMS: number) => {
              if (this.log.logLevel >= LogLevel.SPAM)
                this.log.spam(
                  this.log.buildMessage(
                    () =>
                      '[compAPIcalled] in "waitForFormToShowOnClientThenCallAPI" for API call from server; for API call from server: "' +
                      componentCall.api +
                      '" to form ' +
                      componentCall.form +
                      ', component ' +
              ...
              if (this.log.logLevel >= LogLevel.SPAM)
                this.log.spam(
                  this.log.buildMessage(
                    () =>
                      '[compAPIcalled] in "waitForFormToShowOnClientThenCallAPI" will wait for form ui to load; for API call from server: "' +
                      componentCall.api +
                      '" to form ' +
                      componentCall.form +
                      ', component ' +
              ...
              if (timeOutIfItDoesNotShowMS > 0) {
                ...
                if (this.log.logLevel >= LogLevel.SPAM)
                  this.log.spam(
                    this.log.buildMessage(
                      () =>
                        '[compAPIcalled] api call will time-out if form does not load soon; for API call from server: "' +
                        componentCall.api +
                        '" to form ' +
                        componentCall.form +
                        ', component ' +
                ...
                const timeout = setTimeout(() => {
                  this.log.error(
                    this.log.buildMessage(
                      () =>
                        '[compAPIcalled] Error; non-delayed api call timed out waiting for form UI to load; it will no longer be called; for API call from server: "' +
                        componentCall.api +
                        '" to form ' +
                        componentCall.form +
                        ', component ' +
                  ...
                  () => {
                    if (!cancelled[0]) {
                      this.log.error(
                        this.log.buildMessage(
                          () =>
                            '[compAPIcalled] Error; non-delayed api call cancelled waiting for form UI to load; but the form was destroyed; for API call from server: "' +
                            componentCall.api +
                            '" to form ' +
                            componentCall.form +
                            ', component ' +
            ...
            if (!componentCall.delayUntilFormLoads) {
              if (this.sabloService.isExpectingAFormToShowSoon()) {
                ...
                this.log.error(
                  this.log.buildMessage(
                    () =>
                      '[compAPIcalled] Error calling api ' +
                      componentCall.api +
                      ' in form ' +
                      componentCall.form +
                      ' on component ' +
                      componentCall.bean +
                      '. Form not loaded, no form show expected, and the api cannot be delayed. Api call was skipped.',
                  ),
                );
                return;
              }
  ...
  public static updateComponentModelPropertiesFromServer(
    newComponentProperties: any,
    comp: ComponentCache,
    componentSpec: IWebObjectSpecification,
    ...
    const propertyContextCreator = new RootPropertyContextCreator((propertyName: string) => (comp.model ? comp.model[propertyName] : undefined), componentSpec);
    ...
    for (const propertyName of Object.keys(newComponentProperties)) {
      ...
      newPropertyValue = converterService.convertFromServerToClient(
        newPropertyValue,
        propertyType,
        oldValueOfProperty,
        componentDynamicTypesHolder,
        propertyName,
        propertyContextCreator.withPushToServerFor(propertyName),
      );
  ...
  public static pushEditingStarted(componentModel: Record<string, any>, propertyName: string, sendStartEditFunc: (foundsetLinkedRowId: string, propertyNameToSend: string) => void) {
  ...
  public static pushApplyDataprovider(
    componentModel: Record<string, any>,
    propertyName: string,
    propertyType: IType<any>,
    newValue: any,
    componentSpec: IWebObjectSpecification,
    converterService: ConverterService<unknown>,
    oldValue: any,
  ...
  public destroy(formName: string, triggeredByFormNGOnDestroy: boolean) {
    ...
    if (previousFormUI && !(previousFormUI instanceof Deferred)) this.sabloService.callService('formService', 'formUnloaded', { formname: formName }, true);
    ...
    if (form) {
      ...
      form.formComponents.forEach((fcc) => {
        Object.keys(fcc.model).forEach((propertyName) => {
          this.callOnDestroy(fcc.model[propertyName], afterNgOnDestroyOfChildrenPotentialRunner!, debugLocatorStart ? debugLocatorStart + fcc.name + '.' + propertyName : undefined);
        });
      });
    }
  }
  ...
  private afterNextRenderIfFormUIIsStillPresentRunner(formName: string, fToRun: () => void): void {
    ...
    afterNextRender(
      {
        read: () => {
          if (!this.isFormAngularComponentPresent(formName)) fToRun();
  ...
  public executeEvent(formName: string, componentName: string, handlerName: string, args: IArguments | any[], async?: boolean): Promise<any> {
    ...
    if (form) {
      ...
      if (component && component.model) {
        ...
        promise = wrapPromiseToPropagateCustomRequestInfoInternal(
          promise,
          promise.then(
            // convert return value from server to client
            (retVal) =>
              this.converterService.convertFromServerToClient(
                retVal,
                handlerSpec?.returnType!,
                undefined,
                undefined!,
                undefined!,
    ...
    return this.sabloService.callService('formService', 'executeEvent', cmd, async !== undefined ? async : false).then(
      // convert return value from server to client
      (retVal) =>
        this.converterService.convertFromServerToClient(retVal, handlerSpec?.returnType!, undefined, undefined!, undefined!, PushToServerUtils.PROPERTY_CONTEXT_FOR_INCOMMING_ARGS_AND_RETURN_VALUES),
    );
  }
  ...
  public formWillShow(formname: string, notifyFormVisibility?: boolean, parentForm?: string, beanName?: string, relationname?: string, formIndex?: number): RequestInfoPromise<boolean> {
    ...
    if (notifyFormVisibility) {
      return this.sabloService.callService(
        'formService',
        'formvisibility',
        {
          formname,
  ...
  public hideForm(
    ...
    relationname?: string,
    formIndex?: number,
    formnameThatWillBeShown?: string,
    relationnameThatWillBeShown?: string,
    formIndexThatWillBeShown?: number,
  ): RequestInfoPromise<boolean> {
    if (!formname) {
      throw new Error('formname is undefined');
    }
    let formShow = {};
  ...
  public pushEditingStarted(formName: string, componentName: string, propertyName: string) {
    FormService.pushEditingStarted(this.formsCache.get(formName)!.getComponent(componentName)!.model, propertyName, (foundsetLinkedRowId: string, propertyNameToSend: string) => {
  ...
  private applyDataprovider(formName: string, componentName: string, propertyName: string, newValue: any, oldValue: any) {
    ...
    FormService.pushApplyDataprovider(
      formState.getComponent(componentName)!.model,
      propertyName,
      typeOfData!,
      newValue,
      formState.getComponentSpecification(componentName)!,
  ...
  private formMessageHandler(formCache: FormCache, formName: string, msg: { forms: Record<string, Record<string, Record<string, unknown>>> }, servoyService: ServoyService) {
    ...
    for (const componentName of Object.keys(newFormData)) {
      ...
      FormService.updateComponentModelPropertiesFromServer(
    ...
    children.forEach((elem) => {
      if (elem.layout === true) {
        ...
        if (elem.formComponent && elem.formComponent.length > 0) {
          ...
          const fcc = new FormComponentCache(
            elem.name,
            elem.specName,
            elem.elType,
            elem.handlers,
            elem.responsive,
            elem.position ? elem.position : elem.model.cssPosition,
  ...
  private handleComponentModelConversionsAndChangeListeners(jsonFromServerForComponent: ServerElement, componentCache: ComponentCache, componentSpec: IWebObjectSpecification, formCache: FormCache) {
    ...
    for (const propName of Object.keys(jsonFromServerForComponent.model)) {
      const propValue = (componentCache.model[propName] = this.converterService.convertFromServerToClient(
        jsonFromServerForComponent.model[propName],
        componentSpec?.getPropertyType(propName),
        null,
        componentDynamicTypesHolder,
        propName,
      ...
      if (instanceOfChangeAwareValue(propValue)) {
        propValue.getInternalState().setChangeListener(this.createChangeListenerForSmartValue(formCache.formname, jsonFromServerForComponent.name, propName, propValue));
      }
    }
  }
