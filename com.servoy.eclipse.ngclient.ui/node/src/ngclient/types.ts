...
export const instanceOfApiExecutor = (obj: unknown): obj is IApiExecutor => obj != null && (obj as IApiExecutor).callApi instanceof Function;

export const instanceOfFormComponent = (obj: unknown): obj is IFormComponent => obj != null && (obj as IFormComponent).detectChanges instanceof Function;
...
export class StructureCache implements IRepeaterIDProvider {
  ...
  constructor(
    ...
    public readonly cssPositionContainer?: boolean,
    public layout?: Record<string, string>,
  ) {}
...
export class PartCache implements IRepeaterIDProvider {
  ...
  addChild(child: ComponentCache | FormComponentCache | StructureCache) {
    if (child instanceof ComponentCache && child.type && child.type === 'servoycoreNavigator') return;
    this.items.push(child);
  }
}
...
export class FormComponentCache extends ComponentCache {
  ...
  addChild(child: StructureCache | ComponentCache | FormComponentCache) {
    if (!(child instanceof ComponentCache && (child as ComponentCache).type === 'servoycoreNavigator')) this.items.push(child);
  }
