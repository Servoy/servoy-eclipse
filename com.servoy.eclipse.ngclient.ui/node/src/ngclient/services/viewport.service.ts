...
import {
  ...
  ChangeAwareState,
  isChanged,
  instanceOfChangeAwareValue,
  SubpropertyChangeByReferenceHandler,
  SoftProxyRevoker,
  IUIDestroyAwareValue,
  instanceOfIInternalReferenceBetweenPropertyImpls,
} from '../../sablo/converter.service';
import { IType, ITypeFromServer, IPropertyContext, PushToServerEnum, IWebObjectSpecification, TypesRegistry } from '../../sablo/types_registry';
...
type ColumnCellConversionInfoFromServer = [{ _T: string | null; i: [number] }];
...
export class ViewportService {
   ...
   */
  public updateWholeViewport<T extends any[]>(
    oldViewPort: T,
    internalState: FoundsetViewportState,
    viewPortUpdate: any[],
    ...
    rowCreator?: () => any,
    cellUpdatedFromServerListener?: CellUpdatedFromServerListener,
  ): T {
    ...
    const convertedViewPortUpdate = this.expandTypeInfoAndApplyConversions(
      viewPortUpdateConversions,
      defaultColumnTypes,
      viewPortUpdate,
      0,
      oldViewPort,
  ...
  public updateViewportGranularly(
    viewPort: any[],
    internalState: FoundsetViewportState,
    ...
    for (let i = 0; i < rowUpdates.length; i++) {
      ...
      if (rowUpdate.type === ChangeType.ROWS_CHANGED) {
        ...
        const convertedRowChangeData = this.expandTypeInfoAndApplyConversions(
          rowUpdate._T,
          defaultColumnTypes,
          rowUpdate.rows,
          rowUpdate.startIndex,
          viewPort,
          internalState,
          propertyContextCreator,
          simpleRowValue,
        ...
        for (let j = rowUpdate.startIndex; j < rowUpdate.startIndex + rowUpdate.rows.length; j++) {
          if (worksWithRowLevelProxies) viewPort[j] = this.addRowProxyTo(viewPort, j, propertyContextCreator, internalState);
        ...
        if (internalState.viewportTypes) {
          ...
          for (let j = rowUpdate.startIndex; j <= rowUpdate.endIndex; j++) delete internalState.viewportTypes[j];
        ...
        for (let j = rowUpdate.startIndex; j <= rowUpdate.endIndex; j++) {
          if (worksWithRowLevelProxies) internalState.rowLevelProxyStates![j].softProxyRevoker.getRevokeFunction()(); // disable proxy notifications for deleted row

          this.updateChangeAwareNotifiersForRow(j, viewPort, internalState, simpleRowValue, propertyContextCreator, true);
        }

        if (worksWithRowLevelProxies) internalState.rowLevelProxyStates!.splice(rowUpdate.startIndex, numberOfDeletedRows);
        ...
        for (let j = rowUpdate.startIndex; j < viewPort.length; j++) {
          if (worksWithRowLevelProxies) internalState.rowLevelProxyStates![j].rowIndex = j;

          this.updateChangeAwareNotifiersForRow(j, viewPort, internalState, simpleRowValue, propertyContextCreator, false);
        }
      }

      delete (rowUpdate as any)._T;
      delete (rowUpdate as any).rows; // prepare rowUpdate for listener notifications
    }
  }
  ...
  public getClientSideTypeFor(rowId: any, columnName: string, internalState: FoundsetViewportState, viewPortRowsInCaseCallerIsActualFoundsetProp?: any[]): IType<any> {
    ...
    let clientSideType: IType<any> | undefined;
    if (idx >= 0) {
   ...
   */
  public sendCellChangeToServerBasedOnRowId(
    viewPort: any[],
    internalState: FoundsetViewportState,
    deferredState: IDeferedState,
    rowID: string,
    columnName: string,
    propertyContextCreator: IPropertyContextCreatorForRow,
    newValue: any,
    ...
    if (this.getCellPropertyContextFor(propertyContextCreator, undefined, columnName).getPushToServerCalculatedValue() < PushToServerEnum.ALLOW) {
      internalState.log.spam(
        internalState.log.buildMessage(() => 'svy viewport * sendCellChangeToServerBasedOnRowId denied because pushToServer < ALLOW in .spec (' + rowID + ', ' + columnName + ', ' + newValue),
      );
      return;
    }
    ...
    if (rowIndex === -1) internalState.log.error(internalState.log.buildMessage(() => 'svy viewport * sendCellChangeToServerBasedOnRowId cannot find row index from given rowID ("' + rowID + '"'));

    return this.reallyQueueChange(
      viewPort,
      internalState,
      deferredState,
      rowIndex,
   ...
   */
  private expandTypeInfoAndApplyConversions(
    serverConversionInfo: ConversionInfoFromServerForViewport,
    defaultColumnTypes: IWebObjectSpecification,
    rowsToBeConverted: any[],
    startIdxInViewportForRowsToBeConverted: number,
    oldViewportRows: any[],
    internalState: FoundsetViewportState,
    propertyContextCreator: IPropertyContextCreatorForRow,
    simpleRowValue: boolean,
    fullRowUpdates: boolean,
    rowCreator?: () => any,
    cellUpdatedFromServerListener?: CellUpdatedFromServerListener,
  ): any[] {
    if (rowsToBeConverted) {
      // if it's a simpleRowValue without serverConversionInfo we don't need to alter what we received at all
      rowsToBeConverted.forEach((rowData, index) => {
        if (simpleRowValue) {
          ...
          rowsToBeConverted[index] = this.converterService.convertFromServerToClient(
            rowsToBeConverted[index],
            cellConversion!,
            oldCellVal,
            undefined!,
            undefined!,
          ...
          let rowConversions: Record<string, any> = (
            fullRowUpdates || !internalState.viewportTypes ? undefined : internalState.viewportTypes[startIdxInViewportForRowsToBeConverted + index]
          ) as Record<string, any>;
          const newRowData = rowCreator ? rowCreator() : rowData;

          Object.keys(rowData).forEach((columnName) => {
            let cellConversion: IType<any> | undefined = serverConversionInfo ? this.getCellTypeFromServer(serverConversionInfo, index, columnName) : undefined;
            ...
            const oldCellVal = oldViewportRows
              ? oldViewportRows[startIdxInViewportForRowsToBeConverted + index]
                ? oldViewportRows[startIdxInViewportForRowsToBeConverted + index][columnName]
                : undefined
              : undefined;

            newRowData[columnName] = this.converterService.convertFromServerToClient(
              rowData[columnName],
              cellConversion!,
              oldCellVal,
              undefined! /*dynamic types are already handled via serverConversionInfo here*/,
              undefined!,
  ...
  private getCellTypeFromServer(serverConversionInfo: ConversionInfoFromServerForViewport, rowIndex: number, columnName?: string): IType<any> | undefined {
    ...
    let cellConversion: ITypeFromServer | undefined;
    if (columnTypes) {
      ...
      if (colType) {
        ...
        if (!processed) {
          ...
          if (colType.eT /* cell types */) colType.eT.forEach((value: any) => value.i.forEach((ri: any) => (processed[ri] = value._T)));
        }
  ...
  private updateChangeAwareNotifiersForRow(
    rowIdx: number,
    viewPort: any[],
    internalState: FoundsetViewportState,
    simpleRowValue: boolean,
    propertyContextCreator: IPropertyContextCreatorForRow,
    clearNotifier: boolean,
  ) {
    if (simpleRowValue) {
      this.updateChangeAwareNotifiersForCell(
        viewPort[rowIdx],
        viewPort,
        internalState,
        rowIdx,
      ...
      for (const columnName of Object.getOwnPropertyNames(viewPort[rowIdx])) {
        // (in case of a child component viewport) we only want to iterate on properties from the viewport, not from the model that is not record dependent (that is a prototype of the row here)
        if (columnName !== ViewportService.ROW_ID_COL_KEY && viewPort[rowIdx].propertyIsEnumerable(columnName))
          this.updateChangeAwareNotifiersForCell(
            viewPort[rowIdx][columnName],
            viewPort,
            internalState,
            rowIdx,
            columnName,
  ...
  private updateChangeAwareNotifiersForCell(
    cellValue: any,
    viewPort: any[],
    internalState: FoundsetViewportState,
    rowIdx: number,
    columnName: string,
    propertyContext: IPropertyContext,
    clearNotifier: boolean,
  ) {
    if (instanceOfChangeAwareValue(cellValue)) {
      // child is able to handle it's own change mechanism
      cellValue.getInternalState().setChangeListener(
        clearNotifier
          ? undefined!
          : (_doNotPush?: boolean) => {
  ...
  private reallyQueueChange(
    ...
    try {
      ...
      const convResult = this.converterService.convertFromClientToServer(
        r.value,
        columnName ? (clientSideTypesForRow as Record<string, any>)?.[columnName] : clientSideTypesForRow,
        oldValue,
        propertyContext,
      );
  ...
  private addViewportOrRowProxiesIfNeeded<T extends any[]>(viewPort: T, internalState: FoundsetViewportState, propertyContextCreator: IPropertyContextCreatorForRow, simpleRowValue: boolean): T {
  ...
  private needsRowProxies(internalState: FoundsetViewportState, propertyContextCreator: IPropertyContextCreatorForRow): boolean {
    if (internalState.needsRowProxies !== undefined) return internalState.needsRowProxies;
    ...
    internalState.needsRowProxies = this.getCellPropertyContextFor(propertyContextCreator, undefined, undefined!).getPushToServerCalculatedValue() >= PushToServerEnum.SHALLOW;
  ...
  private getViewportLevelProxyHandler<T extends any[]>(viewPort: T, cellPropertyContext: IPropertyContext, internalState: FoundsetViewportState) {
    ...
    const changeHandlerForWholeViewportProxy: SubpropertyChangeByReferenceHandler = new SubpropertyChangeByReferenceHandler({
      ...
      changeNeedsToBePushedToServer: (viewportIndex: number, oldValue: any, doNotPushNow?: boolean) => {
        this.reallyQueueChange(viewPort, internalState, undefined!, viewportIndex, undefined!, cellPropertyContext, viewPort[viewportIndex], oldValue, doNotPushNow);
      },
    });
  ...
  private getRowLevelProxyHandler(viewPort: any[], initialRowIndex: number, propertyContextCreator: IPropertyContextCreatorForRow, internalState: FoundsetViewportState) {
    ...
    const changeHandlerForRowProxy: SubpropertyChangeByReferenceHandler = new SubpropertyChangeByReferenceHandler({
      ...
      changeNeedsToBePushedToServer: (prop: string, oldValue: any, doNotPushNow?: boolean) => {
        this.reallyQueueChange(
          viewPort,
          internalState,
          undefined!,
          proxyState.rowIndex,
          prop,
          this.getCellPropertyContextFor(propertyContextCreator, viewPort[proxyState.rowIndex], prop),
          viewPort[proxyState.rowIndex][prop],
          oldValue,
          doNotPushNow,
        );
      },
    ...
    return {
      set: (row: any, prop: any, v: any, receiver: any) => {
        ...
        if (instanceOfISomePropertiesInRowAreNotFoundsetLinked(internalState) && !internalState.isFoundsetLinkedProperty(prop)) return Reflect.set(row, prop, v, receiver); // the viewport proxy doesn't want to intercept changes on shared part (that is done elsewhere)
      ...
      deleteProperty: (row: any, prop: any) => {
        ...
        if (instanceOfISomePropertiesInRowAreNotFoundsetLinked(internalState) && !internalState.isFoundsetLinkedProperty(prop)) return Reflect.deleteProperty(row, prop); // the viewport proxy doesn't want to intercept changes on shared part (that is done elsewhere)
        // of the component's model - in case this is a viewport for a component property type
...
export abstract class FoundsetViewportState extends ChangeAwareState implements IUIDestroyAwareValue {
  ...
  constructor(
    public readonly forFoundset: () => IFoundset,
      ...
      afterNgOnDestroyOfChildrenPotentialRunner!(() => {
        if (this.changeListeners?.length) {
          ...
          if (possibleLeaksFound)
            ...
            this.log.warn(
              'svy FoundsetViewportState(' +
                this.constructor.name +
                '): listeners still present after the form UI was destroyed.' +
                ' Check for a possible cumulative memory leak in browser-side-component-code: ' +
                debugLocator,
            );
        }
      });
...
class RowProxyState {
  public constructor(
    public readonly softProxyRevoker: SoftProxyRevoker,
...
const instanceOfISomePropertiesInRowAreNotFoundsetLinked = (obj: any): obj is ISomePropertiesInRowAreNotFoundsetLinked => obj != null && obj.isFoundsetLinkedProperty instanceof Function;
