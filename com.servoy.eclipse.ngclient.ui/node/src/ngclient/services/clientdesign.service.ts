...
export class ClientDesignService {
  ...
  public setFormInDesign(formname: string, names: string[]) {
    ...
    dragresize.ondragend = (isResize, e) => {
      ...
      if (isResize)
        this.sabloService.callService('clientdesign', 'onresize', {
        ...
        this.sabloService.callService('clientdesign', 'ondrop', {
          element: jsevent.elementName,
          location: { x: domRect.left, y: domRect.top },
          size: { width: domRect.width, height: domRect.height },
          formname: formname,
  ...
  recreateUI(formname: string, names: string[]) {
    ...
    if (dragresize) {
      ...
      setTimeout(() => this.setFormInDesign(formname, names));
    }
  }
}
class DragResize {
  ...
  resizeHandleDrag(diffX: number, diffY: number) {
    ...
    const hClass = this.handle && this.handle.className && this.handle.className.match(new RegExp(this.myName + '-([tmblr]{2})')) ? RegExp.$1 : '';
