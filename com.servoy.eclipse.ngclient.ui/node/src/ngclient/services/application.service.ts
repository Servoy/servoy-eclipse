...
export class ApplicationService {
  ...
  public getClientBrowserInformation() {
    ...
    return {
      ...
      timeZone,
      utcOffset: new Date(new Date().getFullYear(), 0, 1, 0, 0, 0, 0).getTimezoneOffset() / -60,
      utcDstOffset: new Date(new Date().getFullYear(), 6, 1, 0, 0, 0, 0).getTimezoneOffset() / -60,
    };
  }
  ...
  public showInfoPanel(url: string, w: number, h: number, t: number, closeText: string) {
    ...
    infoPanel.innerHTML =
      "<iframe marginheight=0 marginwidth=0 scrolling=no frameborder=0 src='" + url + "' width='100%' height='" + (h - 25) + "'></iframe><br><a href='#' id ='closePanelButton'>" + closeText + '</a>';
  ...
  public showDefaultLogin() {
    if (this.localStorageService.get('servoy_username') && this.localStorageService.get('servoy_password')) {
      const promise = this.sabloService.callService(
        'applicationServerService',
        'login',
        { username: this.localStorageService.get('servoy_username'), password: this.localStorageService.get('servoy_password'), encrypted: true },
        false,
      );
      promise.then((ok) => {
        if (!ok) {
  ...
  public generateUploadUrl(formname: string, componentName: string, propertyName: string, tus?: boolean): string {
    ...
    return url + this.sabloService.getClientnr() + (formname ? '/' + formname : '') + (componentName ? '/' + componentName : '') + (propertyName ? '/' + propertyName : '') + '/';
  }
