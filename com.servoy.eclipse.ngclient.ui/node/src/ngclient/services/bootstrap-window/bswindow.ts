...
export class BSWindow {
  ...
  centerWindow() {
    ...
    const maxHeight =
      this.options.references.body.getBoundingClientRect().height -
      bodyTop -
      (this.getInteger(this.options.elements.handle.style.height) + this.getInteger(this.options.elements.footer.style.height)) -
      45;
  ...
  initHandlers() {
    ...
    this.mouseDownListenerHandle = this.renderer.listen(this.options.elements.handle, 'mousedown', (event: any) => {
      ...
      if (this.options.blocker || offY < 5 || handleHeight - offY < 5 || offX < 5 || handleWidth - offX < 5) {
        return;
      }
      this.moving = true;
      this.offset = {};
