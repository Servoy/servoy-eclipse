  ...
  public show(name: string, form: string, title: string) {
    ...
    if (instance) {
      ...
      if (instance.storeBounds) {
        instance.location = this.localStorageService.get(this.servoyService.getSolutionSettings().solutionName + name + '.storedBounds.location');
        instance.size = this.localStorageService.get(this.servoyService.getSolutionSettings().solutionName + name + '.storedBounds.size');
      }
    ...
    this.servoyService.loaded().then(() => {
      ...
      if (this.servoyService.getSolutionSettings().windowName === name) {
        ...
        if (this.appService.getUIProperty('servoy.ngclient.formbased_browser_history') !== false && !isLoginForm) {
          ...
          if (state && state['navigationId']) this.platformLocation.replaceState(form.name, '', this.platformLocation.pathname + this.platformLocation.search + '#' + form.name);
...
export class SvyWindow {
  ...
  setLocation(location: { x: number; y: number }) {
    ...
    if (this.storeBounds) this.windowService.localStorageService.set(this.windowService.servoyService.getSolutionSettings().solutionName + this.name + '.storedBounds.location', this.location);
  }
  ...
  setSize(size: { width: number; height: number }) {
    ...
    if (this.storeBounds) this.windowService.localStorageService.set(this.windowService.servoyService.getSolutionSettings().solutionName + this.name + '.storedBounds.size', this.size);
  }
  ...
  clearBounds() {
    this.windowService.localStorageService.remove(this.windowService.servoyService.getSolutionSettings().solutionName + this.name + '.storedBounds.location');
    this.windowService.localStorageService.remove(this.windowService.servoyService.getSolutionSettings().solutionName + this.name + '.storedBounds.size');
  }
