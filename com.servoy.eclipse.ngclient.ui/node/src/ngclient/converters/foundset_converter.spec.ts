...
describe('FoundsetConverter', () => {
  ...
  beforeEach(() => {
    TestBed.configureTestingModule({
      ...
      providers: [ConverterService, LoggerFactory, WindowRefService, ServicesService, SessionStorageService, ViewportService, LoadingIndicatorService],
    });
  ...
  const createDefaultFoundset = () => {
    ...
    viewport['size'] = 5;
    viewport['rows'] = [{ _svyRowId: '5.ALFKI;_0' }, { _svyRowId: '5.ANATR;_1' }, { _svyRowId: '5.AROUT;_3' }, { _svyRowId: '5.BERGS;_4' }, { _svyRowId: '5.BLAUS;_5' }];
  ...
  it('should convert from server to client', () => {
    fs = converterService.convertFromServerToClient(
      createDefaultFoundset(),
      typesRegistry.getAlreadyRegisteredType(FoundsetType.TYPE_NAME),
      undefined as any,
      undefined as any,
      undefined as any,
      propertyContextWithReject,
  ...
  it('Should scroll down to bottom of foundset - viewport needs to be expanded )', () => {
    ...
    fs.sort([
      { name: 'i', direction: 'asc' },
      { name: 'd', direction: 'desc' },
    ]);
    expect(getAndClearNotified()).toEqual(true);
    expect(fs.getInternalState().hasChanges()).toEqual(true);
    ...
    expect(message).toEqual(
      ...
      ], // id for deferrs is always 1 in tests, does not increase - see TestSabloDeferHelper
    );
    expect(getAndClearNotified()).toEqual(false);
    expect(fs.getInternalState().hasChanges()).toEqual(false);
  });
  ...
  it('Should detect shallow pushToServer cell changes and send them to server )', () => {
    ...
    const serverValue = {
      ...
      selectedRowIndexes: [1],
      multiSelect: false,
      viewPort: { startIndex: 0, size: 9, rows: rows.slice(), _T: { mT: null, cT: { d: { _T: 'Date' } } } },
    };
    fs = converterService.convertFromServerToClient(
      serverValue,
      typesRegistry.getAlreadyRegisteredType(FoundsetType.TYPE_NAME),
