...
import { Deferred, LoggerService, LoggerFactory, RequestInfoPromise, IFoundset, ViewPort, FoundsetChangeEvent, FoundsetChangeListener, IFoundsetFieldsOnly } from '@servoy/public';
...
export class FoundsetType implements IType<FoundsetValue> {
  ...
  constructor(
    ...
    private sabloDeferHelper: SabloDeferHelper,
  ...
  public fromServerToClient(serverJSONValue: ServerSentJSONForFoundset, currentClientValue: FoundsetValue, propertyContext: IPropertyContext): FoundsetValue {
    ...
    if (!serverJSONValue) {
      ...
      if (serverJSONValue.upd_serverSize !== undefined) {
        if (hasListeners)
          notificationParamForListeners!.serverFoundsetSizeChanged = {
      ...
      if (serverJSONValue.upd_hasMoreRows !== undefined) {
        if (hasListeners)
          notificationParamForListeners!.hasMoreRowsChanged = {
      ...
      if (serverJSONValue.upd_multiSelect !== undefined) {
        if (hasListeners)
          notificationParamForListeners!.multiSelectChanged = {
      ...
      if (serverJSONValue.upd_columnFormats !== undefined) {
        if (hasListeners)
          notificationParamForListeners!.columnFormatsChanged = {
      ...
      if (serverJSONValue.upd_sortColumns !== undefined) {
        if (hasListeners)
          notificationParamForListeners!.sortColumnsChanged = {
      ...
      if (serverJSONValue.upd_viewPort !== undefined) {
        ...
        if (viewPortUpdate.startIndex !== undefined && currentClientValue.viewPort.startIndex !== viewPortUpdate.startIndex) {
          if (hasListeners)
            notificationParamForListeners!.viewPortStartIndexChanged = {
        ...
        if (viewPortUpdate.size !== undefined && currentClientValue.viewPort.size !== viewPortUpdate.size) {
          if (hasListeners)
            notificationParamForListeners!.viewPortSizeChanged = {
        ...
        if (viewPortUpdate.rows !== undefined) {
          ...
          currentClientValue.viewPort.rows = this.viewportService.updateWholeViewport(
            currentClientValue.viewPort.rows,
            internalState,
            viewPortUpdate.rows,
            (viewPortUpdate as any)[ConverterService.CONVERSION_CL_SIDE_TYPE_KEY],
            undefined!,
            internalState.propertyContextCreator,
            false,
            () => new RowValue(newValue),
          );

          if (hasListeners)
            notificationParamForListeners!.viewportRowsCompletelyChanged = {
              oldValue: oldRows,
      ...
      if (!serverJSONValue.n && serverJSONValue.serverSize !== undefined) {
        ...
        newValue.viewPort.rows = this.viewportService.updateWholeViewport(
          [] /* this is a full viewport replace; no need to give old/currentClientValue rows here I think */,
          internalState,
          newValue.viewPort.rows,
          newValue.viewPort[ConverterService.CONVERSION_CL_SIDE_TYPE_KEY],
          undefined!,
          internalState.propertyContextCreator,
          false,
          () => new RowValue(newValue),
    ...
    this.log.spam(
      this.log.buildMessage(
        () =>
          'svy foundset * updates or value received from server; new viewport and server size (' +
          (newValue ? newValue.viewPort.startIndex + ', ' + newValue.viewPort.size + ', ' + newValue.serverSize + ', ' + JSON.stringify(newValue.selectedRowIndexes) : newValue) +
          ')',
      ),
    );
...
export class FoundsetValue implements IChangeAwareValue, IFoundset, IUIDestroyAwareValue {
  ...
  public columnDataChangedByRowId(rowID: string, columnName: string, newValue: any, oldValue: any): RequestInfoPromise<any> {
    ...
    return this.__internalState.viewportService.sendCellChangeToServerBasedOnRowId(
      this.viewPort.rows,
      this.__internalState,
      this.__internalState,
      rowID,
      columnName,
      this.__internalState.propertyContextCreator,
      newValue,
      oldValue,
    ) as RequestInfoPromise<any>;
  }
      ...
      afterNgOnDestroyOfChildrenPotentialRunner!(() => {
        ...
        this.viewPort.rows.forEach((row) => {
          if (row._cache) {
            ...
            if (!possibleWarnMessage)
              possibleWarnMessage =
                'svy FoundsetValue: foundset.viewport.rows[?]._cache' +
...
class FoundsetTypeInternalState extends FoundsetViewportState implements IDeferedState {
  ...
  constructor(
    propertyContext: IPropertyContext,
    log: LoggerService,
    public readonly sabloDeferHelper: SabloDeferHelper,
