...
import {
  TypesRegistry,
  ICustomTypesFromServer,
  IPropertiesFromServer,
...
import {
  CustomObjectTypeFactory,
  CustomObjectType,
  ICOTFullValueFromServer,
  ICOTGranularUpdatesToServer,
  BaseCustomObjectState,
  ICOTFullObjectToServer,
  ICOTNoOpToServer,
  ICOTGranularUpdatesFromServer,
  ICOTGranularOpToServer,
} from './json_object_converter';
...
describe('JSONObjectConverter', () => {
  ...
  beforeEach(() => {
    ...
    typesRegistry
      .getTypeFactoryRegistry()
      .contributeTypeFactory(CustomObjectTypeFactory.TYPE_FACTORY_NAME, new CustomObjectTypeFactory(typesRegistry, converterService, specTypesService, loggerFactory));
    ...
    factoryTypeDetails[CustomObjectTypeFactory.TYPE_FACTORY_NAME] = {
      ...
      TabHolder: {
        ...
        tabs: { t: [CustomArrayTypeFactory.TYPE_FACTORY_NAME, [CustomObjectTypeFactory.TYPE_FACTORY_NAME, 'Tab']] as ITypeFromServer, s: 1 } as IPropertyDescriptionFromServerWithMultipleEntries,
      } as IPropertiesFromServer,
      ...
      someTabPanelSpec: {
        p: {
          ...
          untypedObjectALLOWWithVariousSubpropPTS: {
            ...
            s: 1,
          } as IPropertyDescriptionFromServerWithMultipleEntries,
          untypedObjectREJECT: {
          ...
          tabArrayJustForType: {
            t: [CustomArrayTypeFactory.TYPE_FACTORY_NAME, [CustomObjectTypeFactory.TYPE_FACTORY_NAME, 'Tab']] as ITypeFromServer,
            s: 1,
          } as IPropertyDescriptionFromServerWithMultipleEntries,
        },
  ...
  const createSimpleCustomObjectWithUntypedAndSpecificPTSOnSubprops = (): ICOTFullValueFromServer => {
    const subProp = () => ({
      _T: ObjectType.TYPE_NAME,
      _V: {
        x: { _T: ObjectType.TYPE_NAME, _V: ['bla', { _T: DateType.TYPE_NAME_SVY, _V: '2007-12-03T00:00:00' }] }, // this is how ObjectPropertyType sends it JSON from server if it detects nested special types such as Date,
        y: 8,
      },
    });
    return {
  ...
  const createSimpleCustomObjectWithUntypedSubprops = (): ICOTFullValueFromServer => ({
    v: {
      ...
      e: {
        _T: ObjectType.TYPE_NAME,
        _V: {
          x: { _T: ObjectType.TYPE_NAME, _V: ['bla', { _T: DateType.TYPE_NAME_SVY, _V: '2018-01-01T00:00:00' }] },
          y: 8,
        },
  ...
  it('object should be created correctly from server side, and after an update from server it should still not be marked as changed (by client)', () => {
    const val = converterService.convertFromServerToClient(createTabJSON(), oneTabType, undefined as any, undefined as any, undefined as any, getParentPropertyContext(oneTabPushToServer));
    ...
    tab = converterService.convertFromServerToClient(
      { u: [{ k: 'myvalue', v: 'test2' }], vEr: 1 } as ICOTGranularUpdatesFromServer,
      oneTabType,
      tab,
      undefined as any,
      undefined as any,
  ...
  it('object created and changed from client side', () => {
    ...
    const changes: ICOTGranularUpdatesToServer = converterService.convertFromClientToServer(tab, oneTabType, val, getParentPropertyContext(oneTabPushToServer))[0];
  ...
  it('send obj as arg to handler, change a subprop by ref', () => {
    ...
    const changesAndVal: [ICOTFullObjectToServer, Tab] = converterService.convertFromClientToServer(tab, oneTabType, undefined, PushToServerUtils.PROPERTY_CONTEXT_FOR_OUTGOING_ARGS_AND_RETURN_VALUES);
    ...
    const changes2: ICOTGranularUpdatesToServer = converterService.convertFromClientToServer(tab, oneTabType, tab, getParentPropertyContext(oneTabPushToServer))[0];
  ...
  it('send obj from model (with push to server reject) as arg to handler', () => {
    const val = converterService.convertFromServerToClient(
      { v: { sa007: 'test' }, vEr: 1 },
      untypedObjectWithREJECTOnSubpropType,
      undefined as any,
      undefined as any,
      undefined as any,
      getParentPropertyContext(untypedObjectWithREJECTOnSubpropPushToServer),
    );
    ...
    const changesAndVal: [ICOTFullObjectToServer, ICustomObjectValue] = converterService.convertFromClientToServer(
      val,
      untypedObjectWithREJECTOnSubpropType,
      undefined,
    ...
    const changes2: ICOTNoOpToServer = converterService.convertFromClientToServer(
      valCoT,
      untypedObjectWithREJECTOnSubpropType,
      valCoT,
      getParentPropertyContext(untypedObjectWithREJECTOnSubpropPushToServer),
  ...
  it('change subprop. by ref but do not send to server (so it still has changes to send for the model property), then send obj as arg to handler, change another tab by ref; both tabs changed by ref in the model should be then sent to server', () => {
    ...
    const changesAndVal: [ICOTFullObjectToServer, TabHolder] = converterService.convertFromClientToServer(
      tabHolder,
      tabHolderType,
      undefined,
      PushToServerUtils.PROPERTY_CONTEXT_FOR_OUTGOING_ARGS_AND_RETURN_VALUES,
    ...
    const changes2: ICOTGranularUpdatesToServer = converterService.convertFromClientToServer(tabHolder, tabHolderType, tabHolder, getParentPropertyContext(tabHolderPushToServer))[0];
    expect(changes2.vEr).toBe(1);
    expect(changes2.u.length).toBe(1);
  ...
  it('nested object created in scripting', () => {
    ...
    expect((tabHolderSeenInternally.getInternalState() as BaseCustomObjectState<string, any>).contentVersion).toBe(1); // it used to be NaN at some point
    ...
    expect((tabHolderSeenInternally.getInternalState() as BaseCustomObjectState<string, any>).contentVersion).toBe(1);
  ...
  it('nested objects - if somehow marked as allchanged == true should send full values in-depth', () => {
    ...
    const sendToServerResult1 = converterService.convertFromClientToServer(tabHolder, tabHolderType, undefined, getParentPropertyContext(tabHolderPushToServer));
  ...
  it('test deep change in a custom object\'s "object" subprop. with various PTS pe subprop', () => {
    const val: DumbSubpropsWithVariousPTS = converterService.convertFromServerToClient(
      createSimpleCustomObjectWithUntypedAndSpecificPTSOnSubprops(),
      untypedObjectALLOWWithVariousSubpropPTSType,
      undefined as any,
      undefined as any,
      undefined as any,
      getParentPropertyContext(untypedObjectALLOWWithVariousSubpropPTSPushToServer),
    ) as DumbSubpropsWithVariousPTS;
    ...
    let changes2: ICOTNoOpToServer = converterService.convertFromClientToServer(
      val,
      untypedObjectALLOWWithVariousSubpropPTSType,
      val,
      getParentPropertyContext(untypedObjectALLOWWithVariousSubpropPTSPushToServer),
    ...
    let changes: ICOTGranularUpdatesToServer = converterService.convertFromClientToServer(
      val,
      untypedObjectALLOWWithVariousSubpropPTSType,
      val,
      getParentPropertyContext(untypedObjectALLOWWithVariousSubpropPTSPushToServer),
    ...
    changes2 = converterService.convertFromClientToServer(val, untypedObjectALLOWWithVariousSubpropPTSType, val, getParentPropertyContext(untypedObjectALLOWWithVariousSubpropPTSPushToServer))[0];
    expect(changes2.n).toBe(true);
    ...
    changes = converterService.convertFromClientToServer(val, untypedObjectALLOWWithVariousSubpropPTSType, val, getParentPropertyContext(untypedObjectALLOWWithVariousSubpropPTSPushToServer))[0];
  ...
  it('test deep change in a custom object with REJECT "object" subprops', () => {
    const val: DumbSubpropsWithInheritedPTS = converterService.convertFromServerToClient(
      createSimpleCustomObjectWithUntypedSubprops(),
      untypedObjectWithREJECTOnSubpropType,
      undefined as any,
      undefined as any,
      undefined as any,
      getParentPropertyContext(untypedObjectWithREJECTOnSubpropPushToServer),
    ) as DumbSubpropsWithInheritedPTS;
    ...
    changes2 = converterService.convertFromClientToServer(val, untypedObjectWithREJECTOnSubpropType, val, getParentPropertyContext(untypedObjectWithREJECTOnSubpropPushToServer))[0];
    expect(changes2.n).toBe(true);
  });
  ...
  it('test deep change in a custom object with ALLOW "object" subprops', () => {
    const val: DumbSubpropsWithInheritedPTS = converterService.convertFromServerToClient(
      createSimpleCustomObjectWithUntypedSubprops(),
      untypedObjectWithALLOWOnSubpropType,
      undefined as any,
      undefined as any,
      undefined as any,
      getParentPropertyContext(untypedObjectWithALLOWOnSubpropPushToServer),
    ) as DumbSubpropsWithInheritedPTS;
    ...
    changes = converterService.convertFromClientToServer(val, untypedObjectWithALLOWOnSubpropType, val, getParentPropertyContext(untypedObjectWithALLOWOnSubpropPushToServer))[0];
    ...
    changes2 = converterService.convertFromClientToServer(val, untypedObjectWithALLOWOnSubpropType, val, getParentPropertyContext(untypedObjectWithALLOWOnSubpropPushToServer))[0];
    expect(changes2.n).toBe(true);
  });
  ...
  it('test deep change in a custom object with SHALLOW "object" subprops', () => {
    const val: DumbSubpropsWithInheritedPTS = converterService.convertFromServerToClient(
      createSimpleCustomObjectWithUntypedSubprops(),
      untypedObjectWithSHALLOWOnSubpropType,
      undefined as any,
      undefined as any,
      undefined as any,
      getParentPropertyContext(untypedObjectWithSHALLOWOnSubpropPushToServer),
    ) as DumbSubpropsWithInheritedPTS;
    ...
    val.markSubPropertyAsHavingDeepChanges!('b');
    checkNotifiedAndTriggeredAndClear(true, false); // DEEP change in element that was marked manually; but as it is SHALLOW it does no send it right away to server
    changes = converterService.convertFromClientToServer(val, untypedObjectWithSHALLOWOnSubpropType, val, getParentPropertyContext(untypedObjectWithSHALLOWOnSubpropPushToServer))[0];
    ...
    changes2 = converterService.convertFromClientToServer(val, untypedObjectWithSHALLOWOnSubpropType, val, getParentPropertyContext(untypedObjectWithSHALLOWOnSubpropPushToServer))[0];
    expect(changes2.n).toBe(true);
  });
  ...
  it('test deep change in a custom object with DEEP "object" subprops', () => {
    const val: DumbSubpropsWithInheritedPTS = converterService.convertFromServerToClient(
      createSimpleCustomObjectWithUntypedSubprops(),
      untypedObjectWithDEEPOnSubpropType,
      undefined as any,
      undefined as any,
      undefined as any,
      getParentPropertyContext(untypedObjectWithDEEPOnSubpropPushToServer),
    ) as DumbSubpropsWithInheritedPTS;
    ...
    val.b.a = false;
    checkNotifiedAndTriggeredAndClear(false, false); // DEEP change in element - in untyped object; we do not automatically detect those
    let changes2: ICOTNoOpToServer = converterService.convertFromClientToServer(val, untypedObjectWithDEEPOnSubpropType, val, getParentPropertyContext(untypedObjectWithDEEPOnSubpropPushToServer))[0];
    expect(changes2.n).toBe(true);
    ...
    changes = converterService.convertFromClientToServer(val, untypedObjectWithDEEPOnSubpropType, val, getParentPropertyContext(untypedObjectWithDEEPOnSubpropPushToServer))[0];
    ...
    changes2 = converterService.convertFromClientToServer(val, untypedObjectWithDEEPOnSubpropType, val, getParentPropertyContext(untypedObjectWithDEEPOnSubpropPushToServer))[0];
    expect(changes2.n).toBe(true);
  });
  ...
  it('legacy TabDeprecated created from server with correct instance, with old api available', () => {
    ...
    for (const k in tab) if (k === 'constructor' || k === 'get2ConcattedProps') expect.fail("'" + k + "' should not be an enumerable property!");
  ...
  it('legacy TabDeprecated created from client with correct instance, should get new impl. as well after send to server', () => {
    ...
    [, tab] = converterService.convertFromClientToServer(tab, oneTabType, undefined, getParentPropertyContext(oneTabPushToServer));
  ...
  it('when an already smart value (received as return value from an server side api call for example) is assigned into the model into a new location and sent to server, it should still work - have a correct change listener etc.', () => {
    ...
    const childArray = converterService.convertFromServerToClient(
      { v: [createTabJSON()], vEr: 1 },
      tabArrayType,
      undefined as any,
      undefined as any,
      undefined as any,
      PushToServerUtils.PROPERTY_CONTEXT_FOR_INCOMMING_ARGS_AND_RETURN_VALUES,
    ) as Tab[];
    ...
    // simulate a send to server as argument to a handler for this array (oldVal undefined) - to make sure it doesn't messup it's state if it's also a model prop. (it used getParentPropertyContext above which is for a model prop)
    const changesAndVal: [ICOTGranularUpdatesToServer, any] = converterService.convertFromClientToServer(tabHolder, tabHolderType, tabHolder, getParentPropertyContext(tabHolderPushToServer));
    const changes = changesAndVal[0];
...
class TabDeprecated extends BaseCustomObject {
  // test deprecated scenario as well
