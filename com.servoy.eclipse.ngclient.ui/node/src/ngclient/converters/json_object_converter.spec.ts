...
describe( 'JSONObjectConverter', () => {
    ...
    beforeEach(() => {
        ...
        converterService.getSpecTypesService().registerType( "TabHolder", TabHolder, ["id", "tab", "tab2", "tab3"] );
    } );
    ...
    it( 'nested objects - if somehow marked as allchanged == true should send full values in-depth', () => {
        ...
        tabHolder.getStateHolder().allChanged = true; // could be set at runtime for example by a parent custom object or custom array type when that one wants to be sent fully
        changes = converterService.convertFromClientToServer( tabHolder, "JSON_obj" )
