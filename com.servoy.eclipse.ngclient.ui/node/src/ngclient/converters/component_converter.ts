import { ConverterService, isChanged, IChangeAwareValue, instanceOfChangeAwareValue, ChangeListenerFunction, IUIDestroyAwareValue, instanceOfUIDestroyAwareValue } from '../../sablo/converter.service';
...
import {
  FoundsetViewportState,
  ViewportService,
  IPropertyContextCreatorForRow,
  ISomePropertiesInRowAreNotFoundsetLinked,
  ConversionInfoFromServerForViewport,
  RowUpdate,
...
export class ComponentType implements IType<ChildComponentPropertyValue> {
  ...
  constructor(
    ...
    private readonly sabloService: SabloService,
  ...
  public fromServerToClient(serverSentData: IServerSentData, currentClientValue: ChildComponentPropertyValue, propertyContext: IPropertyContext): ChildComponentPropertyValue {
    ...
    } else if (!serverSentData || !serverSentData.n) {
      // .n means NO OPERATION
      // full contents received
      if (serverSentData) {
        newValue = new ChildComponentPropertyValue(
          serverSentData,
          currentClientValue,
          propertyContext,
...
export class ChildComponentPropertyValue extends ComponentCache implements IChangeAwareValue, IChildComponentPropertyValue, IUIDestroyAwareValue {
  ...
  constructor(
    serverSentData: IServerSentData,
    oldClientValue: ChildComponentPropertyValue,
    ...
    this.__internalState = new ComponentTypeInternalState(
      this,
      ...
      log,
      this.typesRegistry,
      forFoundsetPropertyName ? () => propertyContext.getProperty(forFoundsetPropertyName) : undefined,
    );
  ...
  startEdit(propertyName: string, rowId?: any) {
    ...
    FormService.pushEditingStarted(this.__internalState.getActualComponentModel(rowId), propertyName, (foundsetLinkedRowId: string, propertyNameToSend: string) => {
...
class ComponentTypeInternalState extends FoundsetViewportState implements ISomePropertiesInRowAreNotFoundsetLinked {
  ...
  constructor(
    private readonly componentValue: ChildComponentPropertyValue,
  ...
  initializeFullValueFromServer(serverSentData: IServerSentData): void {
    FormService.updateComponentModelPropertiesFromServer(
      ...
      (propertiesChangedButNotByRef: { propertyName: string; newPropertyValue: any }[]) =>
        this.componentValue.triggerNgOnChangeWithSameRefDueToSmartPropertyUpdate
          ? this.sabloService.addIncomingMessageHandlingDoneTask(() => {
              if (this.componentValue.triggerNgOnChangeWithSameRefDueToSmartPropertyUpdate) this.componentValue.triggerNgOnChangeWithSameRefDueToSmartPropertyUpdate(propertiesChangedButNotByRef, -1);
            })
          : undefined,
    );
    ...
    this.applyUpdatesAndHandleCellChangesThatAreNotByRef((cellUpdatedFromServerListener: (relativeRrowIndex: number, columnName: string, oldValue: any, newValue: any) => void) => {
      if (wholeViewportUpdateFromServer) {
        this.componentValue.modelViewport = this.viewportService.updateWholeViewport(
          this.componentValue.modelViewport,
          this,
          wholeViewportUpdateFromServer,
          serverSentData._T!,
          this.componentSpecification,
          this.propertyContextCreatorForRow,
          false,
          this.getRowModelCreator(),
          cellUpdatedFromServerListener,
        );
      }

      if (serverSentData.handlers)
        for (const handlerName of Object.keys(serverSentData.handlers)) this.componentValue.mappedHandlers.set(handlerName, this.generateWrapperHandler(handlerName, this.componentValue));
  ...
  sendChanges(propertyName: string, newValue: any, oldValue: any, rowId?: string, isDataprovider?: boolean) {
    ...
    if (isDataprovider) {
      ...
      if (!isChanged(newValue, oldValue)) {
        ...
        const converted: [any, any] = this.converterService.convertFromClientToServer(
          newValue,
          clientSideType,
          oldValue,
      ...
      this.viewportService.sendCellChangeToServerBasedOnRowId(this.componentValue.modelViewport, this, undefined!, rowId!, propertyName, this.propertyContextCreatorForRow, newValue, oldValue);
    }
  }
  ...
  private generateWrapperHandler(handlerName: string, componentValue: ChildComponentPropertyValue) {
    const executeHandler = (args: IArguments, rowId: string) => {
      ...
      return deferred.deferred.promise.then((retVal) =>
  ...
  private applyUpdatesAndHandleCellChangesThatAreNotByRef(
    applyUpdatesFunction: (cellUpdatedFromServerListener: (relativeRowIndex: number, columnName: string, oldValue: any, newValue: any) => void) => void,
  ) {
  ...
  public override uiDestroyed(afterNgOnDestroyOfChildrenPotentialRunner?: (f: () => void) => void, debugLocator?: string): void {
    ...
    modelKeys.forEach((key) => {
      ...
      if (instanceOfUIDestroyAwareValue(v)) v.uiDestroyed(afterNgOnDestroyOfChildrenPotentialRunner, debugLocator ? debugLocator + '.model.' + key : undefined);
    });
    ...
    this.componentValue.modelViewport.forEach((row, rowIdx) => {
      for (const key of Object.keys(row)) {
        if (!modelKeySet.has(key)) {
          // only check properties that are not in model as those are shared and already checked above
          const v = row[key];
          if (instanceOfUIDestroyAwareValue(v)) v.uiDestroyed(afterNgOnDestroyOfChildrenPotentialRunner, debugLocator ? debugLocator + '.modelViewport.rows[' + rowIdx + '].' + key : undefined);
        }
      }
    });
