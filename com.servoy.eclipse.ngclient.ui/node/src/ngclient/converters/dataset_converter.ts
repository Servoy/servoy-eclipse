  ...
  fromServerToClient(serverJSONValue: any, _currentClientValue: any, propertyContext: IPropertyContext): any {
    ...
    if (datasetValue) {
      ...
      if (columnTypesFromServer) {
        ...
        columnTypes = {};
        for (const colIdx of Object.getOwnPropertyNames(columnTypesFromServer)) columnTypes[colIdx] = this.typesRegistry.getAlreadyRegisteredType(columnTypesFromServer[colIdx])!;
      }
      ...
      while (rowNo < datasetValue.length) {
        ...
        row.forEach((cellValue: any, columnIndex: number) => {
          ...
          row[columnIndex] = this.converterService.convertFromServerToClient(cellValue, columnTypes ? columnTypes[columnIndex] : undefined, undefined, undefined!, undefined!, propertyContext);
        });
        rowNo++;
      }
    }
