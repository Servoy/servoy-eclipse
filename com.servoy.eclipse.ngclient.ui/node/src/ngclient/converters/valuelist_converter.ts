...
export class ValuelistType implements IType<Valuelist> {
  ...
  fromServerToClient(serverJSONValue: IValuelistTValueFromServer, currentClientValue: Valuelist, _propertyContext: IPropertyContext): Valuelist {
    ...
    if (serverJSONValue) {
      ...
      if (serverJSONValue.values) {
        ...
        if (!newValue) {
          ...
          internalState.realToDisplayCache = currentClientValue && currentClientValue.getInternalState() ? currentClientValue.getInternalState().realToDisplayCache : internalState.realToDisplayCache;
      ...
      if (serverJSONValue.handledID) {
        ...
        this.sabloDeferHelper.resolveDeferedEvent(
          handledIDAndState.id,
          newValue!.getInternalState(),
          handledIDAndState.value ? deferredValue : 'No value for ' + handledIDAndState.id,
          handledIDAndState.value,
        );
      }
    } else {
      newValue = null!;
...
export class Valuelist extends Array<{ displayValue: string; realValue: any }> implements IValuelist, IChangeAwareValue, IUIDestroyAwareValue {
  constructor(
    ...
    private realValueType: string,
    private internalState: ValuelistState,
    values?: { displayValue: string; realValue: any }[],
  ) {
    super();
    ...
    if (realValue != null && realValue !== undefined) {
      if (this.internalState.valuelistid === undefined) {
        ...
        if (promiseOrValue !== undefined) {
          if (promiseOrValue instanceof Promise) return from(promiseOrValue);
        ...
        this.internalState.realToDisplayCache[key] = this.internalState.deferred[this.internalState.diplayValueReq.id].defer.promise
          .then((val) => {
