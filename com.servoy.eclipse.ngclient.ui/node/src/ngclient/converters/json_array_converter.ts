...
export class CustomArrayTypeFactory implements ITypeFactory<CustomArrayValue<any>> {
  ...
  getOrCreateSpecificType(specificElementInfo: ITypeFromServer | { t: ITypeFromServer; s: PushToServerEnumServerValue }, webObjectSpecName: string): CustomArrayType<any> {
    ...
    const elementTypeFromSrv: ITypeFromServer = elementTypeWithNoPushToServer
      ? (specificElementInfo as ITypeFromServer)
      : (specificElementInfo as { t: ITypeFromServer; s: PushToServerEnumServerValue }).t;
    const pushToServer: PushToServerEnum | undefined = elementTypeWithNoPushToServer
      ? undefined
      : PushToServerUtils.valueOf((specificElementInfo as { t: ITypeFromServer; s: PushToServerEnumServerValue }).s);
...
export class CustomArrayType<T> implements IType<CustomArrayValue<T>> {
  constructor(
    private readonly staticElementType: IType<any>,
  ...
  fromServerToClient(serverJSONValue: ICATDataFromServer, currentClientValue: CustomArrayValue<T>, propertyContext: IPropertyContext): CustomArrayValue<T> {
    ...
    let internalState: ArrayState | null = null;

    const elemPropertyContext = propertyContext
      ? new PropertyContext(
          propertyContext.getProperty,
          PushToServerUtils.combineWithChildStatic(propertyContext.getPushToServerCalculatedValue(), this.pushToServerForElements),
      ...
      if (instanceOfFullValueFromServer(serverJSONValue)) {
        ...
        if (newValue.length) {
          for (let c = 0; c < newValue.length; c++) {
            ...
            newValue[c] = elem = this.converterService.convertFromServerToClient(
              elem,
              this.staticElementType,
          ...
          for (const granularOp of serverJSONValue.g) {
            ...
            if (opType === ICATOpTypeEnum.CHANGED) {
              ...
              for (let i = startIndex; i <= endIndex; i++) {
                ...
                currentClientValue[i] = this.converterService.convertFromServerToClient(
                  changedData[relIdx],
                  this.staticElementType,
                  currentClientValue[i],
                  internalState.dynamicPropertyTypesHolder,
                  '' + i,
                  elemPropertyContext!,
                ) as T;
              ...
              for (let i = numberOfInsertedRows - 1; i >= 0; i--) {
                const addedRow = this.converterService.convertFromServerToClient(
                  insertedData[i],
                  this.staticElementType,
                  undefined,
                  internalState.dynamicPropertyTypesHolder,
  ...
  fromClientToServer(newClientData: any, oldClientData: CustomArrayValue<any>, propertyContext: IPropertyContext): [ICATDataToServer, CustomArrayValue<any>] | null {
    ...
    let internalState: ArrayState | undefined;
    try {
      if (newClientData) {
        ...
        if (!propertyContext?.isInsideModel || internalState!.hasChanges()) {
          ...
          if (!propertyContext?.isInsideModel || internalState!.hasFullyChanged()) {
            ...
            for (let idx = 0; idx < newClientDataInited.length; idx++) {
              ...
              const converted = this.converterService.convertFromClientToServer(val, this.getElementType(internalState!, idx), oldClientData ? oldClientData[idx] : undefined, elemPropertyContext!);
              ...
              if (instanceOfChangeAwareValue(converted[1])) converted[1].getInternalState().setChangeListener(this.getChangeListener(newClientDataInited, idx));
  ...
  private initArrayValue(arrayToInitialize: any[], contentVersion: number, pushToServerCalculatedValue: PushToServerEnum, force?: boolean): CustomArrayValue<T> {
...
export class ArrayState extends BaseCustomObjectState<number, any[]> {
  constructor(
    ...
    public readonly pushToServerForElements: PushToServerEnum,
  ) {
    super(originalNonProxiedInstanceOfCustomObject);
  }
}

const instanceOfCustomArray = <T>(obj: any): obj is CustomArrayValue<T> => instanceOfChangeAwareValue(obj) && obj.getInternalState() instanceof ArrayState;
...
class CustomArrayValue<T> extends Array<T> implements ICustomArrayValue<T>, IChangeAwareValue, IUIDestroyAwareValue, ICustomArray<T> {
  ...
  markElementAsHavingDeepChanges(index: number): void {
    ...
    const pushToServerOnElements = PushToServerUtils.combineWithChildStatic(this.__internalState.calculatedPushToServerOfWholeProp, this.__internalState.pushToServerForElements);
  ...
  public uiDestroyed(afterNgOnDestroyOfChildrenPotentialRunner?: (f: () => void) => void, debugLocator?: string): void {
    ...
    if (this.length) {
      for (let c = 0; c < this.length; c++) {
        const elem = this[c];
        if (instanceOfUIDestroyAwareValue(elem)) elem.uiDestroyed(afterNgOnDestroyOfChildrenPotentialRunner, debugLocator ? debugLocator + '[' + c + ']' : undefined);
      }
    }
  }
}
...
const instanceOfFullValueFromServer = (obj: any): obj is ICATFullValueFromServer => obj && !!(obj as ICATFullValueFromServer).v;

const instanceOfGranularUpdatesFromServer = (obj: any): obj is ICATGranularUpdatesFromServer => obj && !!(obj as ICATGranularUpdatesFromServer).g;

const instanceOfNoOpFromServer = (obj: any): obj is ICATNoOpFromServer => obj && !!(obj as ICATNoOpFromServer).n;
