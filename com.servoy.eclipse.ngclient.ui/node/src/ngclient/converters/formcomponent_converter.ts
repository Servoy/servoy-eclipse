import { ChangeAwareState, ConverterService, IChangeAwareValue, instanceOfChangeAwareValue, IUIDestroyAwareValue } from '../../sablo/converter.service';
...
export class FormcomponentType implements IType<FormComponentValue> {
  ...
  fromServerToClient(serverSentData: any, currentClientData: FormComponentValue, propertyContext: IPropertyContext): FormComponentValue {
    ...
    if (!formComponentPropertyValue)
      formComponentPropertyValue = new FormComponentValue(
        serverSentData.absoluteLayout,
    ...
    if (serverSentData.childElements) {
      ...
      for (let idx = 0; idx < serverSentData.childElements.length; idx++) {
        ...
        formComponentPropertyValue.childElements[idx] = childCompElem = this.converterService.convertFromServerToClient(
          serverSentData.childElements[idx],
          ...
          undefined!,
          propertyContext,
        ) as ChildComponentPropertyValue;
  ...
  fromClientToServer(newClientData: FormComponentValue, oldClientData: FormComponentValue, propertyContext: IPropertyContext): [any, FormComponentValue] | null {
    ...
    if (newClientData.childElements) {
      ...
      for (let idx = 0; idx < newClientData.childElements.length; idx++) {
        changes[idx] = this.converterService.convertFromClientToServer(
          newClientData.childElements[idx],
          this.typesRegistry.getAlreadyRegisteredType(ComponentType.TYPE_NAME),
          oldClientData && oldClientData.childElements ? oldClientData.childElements[idx] : null,
          propertyContext,
        )[0];
      }
    }
...
export class FormComponentValue extends ChangeAwareState implements IChangeAwareValue, IUIDestroyAwareValue {
  ...
  public uiDestroyed(afterNgOnDestroyOfChildrenPotentialRunner?: (f: () => void) => void, debugLocator?: string): void {
    ...
    this.childElements?.forEach((child, idx) => child?.uiDestroyed(afterNgOnDestroyOfChildrenPotentialRunner, debugLocator ? debugLocator + '.childElements[' + idx + ']' : undefined));
  }
}
