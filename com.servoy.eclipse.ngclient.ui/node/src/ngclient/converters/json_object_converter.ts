import {
  ...
  IParentAccessForSubpropertyChanges,
  IChangeAwareValue,
...
import {
  ...
  ITypesRegistryForTypeFactories,
  PushToServerUtils,
  PropertyContext,
  ChildPropertyContextCreator,
  IPropertyContextGetterMethod,
...
export class CustomObjectTypeFactory implements ITypeFactory<CustomObjectValue> {
  ...
  constructor(
    private readonly typesRegistry: ITypesRegistryForTypeFactories,
    private readonly converterService: ConverterService<unknown>,
    private readonly specTypesService: SpecTypesService,
  ...
  getOrCreateSpecificType(specificTypeInfo: string, webObjectSpecName: string): CustomObjectType {
    ...
    if (customTypesForThisSpec) {
      ...
      if (!coType)
        this.logger.error(
          '[CustomObjectTypeFactory] cannot find custom object client side type "' +
            specificTypeInfo +
            '" for spec "' +
            webObjectSpecName +
...
class CustomObjectValue implements IChangeAwareValue, ICustomObjectValue, IUIDestroyAwareValue {
  ...
  markSubPropertyAsHavingDeepChanges(subPropertyName: string): void {
    ...
    const pushToServerOnSubProp = PushToServerUtils.combineWithChildStatic(
      this.__internalState.calculatedPushToServerOfWholeProp,
  ...
  public uiDestroyed(afterNgOnDestroyOfChildrenPotentialRunner?: (f: () => void) => void, debugLocator?: string): void {
    ...
    for (const c of Object.keys(this)) {
      ...
      if (instanceOfUIDestroyAwareValue(subPropValue)) subPropValue.uiDestroyed(afterNgOnDestroyOfChildrenPotentialRunner, debugLocator ? debugLocator + '.' + c : undefined);
    }
  }
}
...
export class CustomObjectType implements IType<CustomObjectValue> {
  ...
  constructor(
    ...
    private readonly specTypesService: SpecTypesService,
    private readonly logger: LoggerService,
    private readonly fullyQulifiedTypeName: string,
  ) {}
    ...
    try {
      if (instanceOfFullValueFromServer(serverJSONValue)) {
        ...
        for (const c of Object.keys(newValue)) {
          ...
          newValue[c] = subPropValue = this.converterService.convertFromServerToClient(
            subPropValue,
            this.getStaticPropertyType(c),
            currentClientValue ? currentClientValue[c] : undefined,
            internalState.dynamicPropertyTypesHolder,
            c,
            propertyContextCreator.withPushToServerFor(c),
          );
        ...
        if (internalState.contentVersion === serverJSONValue.vEr) {
          ...
          for (const update of updates) {
            ...
            currentClientValue[key] = val = this.converterService.convertFromServerToClient(
              val,
              this.getStaticPropertyType(key),
              currentClientValue[key],
              internalState.dynamicPropertyTypesHolder,
              key,
    ...
    try {
      if (newClientData) {
        if (!instanceOfCustomObject(newClientData)) {
          ...
          internalState.ignoreChanges = true;
        } else if (propertyContext?.isInsideModel && (newClientData !== oldClientData || !newClientData.getInternalState().hasChangeListener())) {
          // the hasChangeListener is meant to detect arguments that come as a api call to client and then are assigned to a property - and because of the now deprecated ServoyPublicService.sendServiceChanges that did not have an oldPropertyValue argument, here old was === new value and we couldn't detect it correctly as a change by ref
          ...
          // an already initialized value that is either the same value as before or it is used here as an argument or return value to api calls/handlers
      ...
      if (newClientDataInited) {
        ...
        if (propertyContext.isInsideModel) {
          internalState!.calculatedPushToServerOfWholeProp =
            typeof propertyContext?.getPushToServerCalculatedValue() != 'undefined' ? propertyContext?.getPushToServerCalculatedValue() : PushToServerEnum.REJECT;
        ...
        if (!propertyContext?.isInsideModel || internalState!.hasChanges()) {
          // so either it has changes or it's used as an arg/return value to a handler/api call
          const changes = {} as ICOTFullObjectToServer | ICOTGranularUpdatesToServer;
          if (!propertyContext?.isInsideModel || internalState!.hasFullyChanged()) {
            // fully changed or arg/return value of handler/api call
            ...
            for (const key of Object.keys(newClientDataInited)) {
              ...
              const converted = this.converterService.convertFromClientToServer(
                val,
                this.getPropertyType(internalState!, key),
                oldClientData ? oldClientData[key] : undefined,
                propertyContextCreator.withPushToServerFor(key),
              );
              ...
              if (instanceOfChangeAwareValue(converted[1])) converted[1].getInternalState().setChangeListener(this.getChangeListener(newClientDataInited, key));

              if (PushToServerUtils.combineWithChildStatic(calculatedPushToServerOfWholeProp, this.propertyDescriptions[key]?.getPropertyDeclaredPushToServer()) === PushToServerEnum.REJECT)
                delete toBeSentObj[key]; // don't send to server pushToServer reject keys
            }
            ...
            for (const key of internalState!.changedKeys.keys()) {
              ...
              if (changed) {
                ...
                const converted = this.converterService.convertFromClientToServer(newVal, this.getPropertyType(internalState!, key), oldVal, propertyContextCreator.withPushToServerFor(key));
  ...
  private getCustomObjectPropertyContextGetter(customObjectValue: CustomObjectValue, parentPropertyContext: IPropertyContext): IPropertyContextGetterMethod {
    ...
    return (propertyName) => {
      if (customObjectValue.hasOwnProperty(propertyName))
        return customObjectValue[propertyName]; // can even be null or undefined as long as it is set on this object
  ...
  private static clonePrototypeDeep(p: any): [any, any] {
    ...
    let lastClonedP: any;
    if (protToUseAfterCloning && protToUseAfterCloning !== Object.prototype) [protToUseAfterCloning, lastClonedP] = CustomObjectType.clonePrototypeDeep(protToUseAfterCloning);

    // shallow copy of "p"
    const clonedP = Object.create(protToUseAfterCloning, Object.getOwnPropertyDescriptors(p));
  ...
  private initCustomObjectValue(objectToInitialize: any, contentVersion: number, pushToServerCalculatedValue: PushToServerEnum, force?: boolean): CustomObjectValue {
    ...
    if (!instanceOfChangeAwareValue(objectToInitialize) || force) {
      ...
      if (protoOfObj !== CustomObjectValue.prototype && protoOfObj.constructor !== CustomObjectValue.prototype.constructor) {
        // so check if CustomObjectValue or a clone  is already there / either directly or as a clone (that has the same constructor)
  ...
  private getProxyHandler(internalState: CustomObjectState) {
    ...
    return {
      set: (underlyingCustomObject: CustomObjectValue, prop: any, v: any, receiver: any) => {
        ...
        const subPropCalculatedPushToServer = PushToServerUtils.combineWithChildStatic(
          internalState.calculatedPushToServerOfWholeProp,
...
const instanceOfCustomObject = (obj: any): obj is CustomObjectValue => instanceOfChangeAwareValue(obj) && obj.getInternalState() instanceof CustomObjectState;

export interface BCOSBackup extends CASBackup {
  dynamicPropertyTypesHolder: Record<string, any>;
}

export abstract class BaseCustomObjectState<KeyT extends number | string, VT> extends ChangeAwareState implements IParentAccessForSubpropertyChanges<KeyT> {
...
class CustomObjectState extends BaseCustomObjectState<any, any> {
  constructor(
    originalNonProxiedInstanceOfCustomObject: any,
    public readonly propertyDescriptions: Record<string, IPropertyDescription>,
  ) {
...
const instanceOfFullValueFromServer = (obj: any): obj is ICOTFullValueFromServer => obj && !!(obj as ICOTFullValueFromServer).v;

const instanceOfGranularUpdatesFromServer = (obj: any): obj is ICOTGranularUpdatesFromServer => obj && !!(obj as ICOTGranularUpdatesFromServer).u;

const instanceOfNoOpFromServer = (obj: any): obj is ICOTNoOpFromServer => obj && !!(obj as ICOTNoOpFromServer).n;
