...
export class FoundsetLinkedValue extends Array<any> implements IChangeAwareValue, IUIDestroyAwareValue {
  ...
  public constructor(viewportService: ViewportService, sabloService: SabloService, propertyContext: IPropertyContext, forFoundset: () => IFoundset, log: LoggerService) {
    super();
  ...
  public dataChanged(index: number, newValue: any, oldValue?: any) {
    ...
    this.__internalState.viewportService.sendCellChangeToServerBasedOnRowId(
      this,
      this.__internalState,
      undefined!,
      this.__internalState.forFoundset().viewPort.rows[index]._svyRowId,
      undefined!,
      this.__internalState.getPropertyContextCreatorForRow(),
      newValue,
      oldValue,
    );
  }
...
class FSLinkedInternalState extends FoundsetViewportState {
  ...
  constructor(
    private foundsetLinkedValue: FoundsetLinkedValue,
  ...
  applyIncommingServerData(didSomethingWithServerSentData: boolean, serverSentData: IServerSentData): FoundsetLinkedValue {
    if (serverSentData != null && serverSentData !== undefined) {
      ...
      if (serverSentData.vpu !== undefined) {
        ...
        this.viewportService.updateViewportGranularly(this.foundsetLinkedValue, this, serverSentData.vpu, undefined!, this.propertyContextCreatorForRow, true);
        ...
        if (serverSentData.sv !== undefined || serverSentData.svu !== undefined) {
          // SINGLE_VALUE or SINGLE_VALUE_UPDATE
  ...
  public updateWholeViewport(wholeViewport: any[], conversionInfo: ConversionInfoFromServerForViewport | undefined) {
    ...
    this.foundsetLinkedValue = this.viewportService.updateWholeViewport(this.foundsetLinkedValue, this, wholeViewport, conversionInfo!, undefined!, this.propertyContextCreatorForRow, true);
  ...
  public override uiDestroyed(afterNgOnDestroyOfChildrenPotentialRunner?: (f: () => void) => void, debugLocator?: string): void {
    ...
    if (this.foundsetLinkedValue) {
      this.foundsetLinkedValue.forEach((row, rowIdx) => {
        if (instanceOfUIDestroyAwareValue(row)) row.uiDestroyed(afterNgOnDestroyOfChildrenPotentialRunner, debugLocator ? debugLocator + '[' + rowIdx + ']' : undefined);
      });
    }
...
class SingleValueState implements IUIDestroyAwareValue {
  ...
  constructor(
    ...
    private iS: FSLinkedInternalState,
  ) {
    ...
    sabloService.addIncomingMessageHandlingDoneTask(() => {
      // do it after all incomming properties have been converted so we are sure to have the forFoundset prop. ready
  ...
  public uiDestroyed(afterNgOnDestroyOfChildrenPotentialRunner?: (f: () => void) => void, debugLocator?: string): void {
    ...
    if (instanceOfUIDestroyAwareValue(this.singleValue)) this.singleValue.uiDestroyed(afterNgOnDestroyOfChildrenPotentialRunner, debugLocator);
  }
