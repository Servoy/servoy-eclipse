...
@Component({
  ...
  template: `
    <div class="svy-listformcomponent" [class]="styleClass()" #element>
      @if (useScrolling) {
        <ag-grid-angular #aggrid [gridOptions]="agGridOptions" [style]="getAGGridStyle()" [sabloTabseq]="tabSeq()" [sabloTabseqConfig]="{ container: true, reservedGap: 1000 }"> </ag-grid-angular>
      }
      ...
      @if (!useScrolling) {
          ...
          <div>
            @for (row of getViewportRows(); track row; let i = $index) {
              <div
                ...
                [style]="{ height: getRowHeight() !== null ? getRowHeight() + 'px' : null, width: getRowWidth() }"
                style="display:inline-block; position: relative"
              >
                @for (item of cache.items; track item) {
                  <div [svyContainerStyle]="item" [svyContainerLayout]="item.layout" class="svy-wrapper" style="position:absolute">
                    <!-- wrapper div -->
        ...
        <div class="svyPagination">
          <div
            #firstelement
            style="text-align:center;cursor:pointer;display:inline;visibility:hidden;padding:3px;white-space:nowrap;vertical-align:middle;background-color:rgb(255, 255, 255, 0.6);"
            (click)="firstPage()"
          >
          ...
          <div
            #leftelement
            style="text-align:center;cursor:pointer;visibility:hidden;display:inline;padding:3px;white-space:nowrap;vertical-align:middle;background-color:rgb(255, 255, 255, 0.6);"
            (click)="moveLeft()"
          >
    ...
    <ng-template #formComponentAbsoluteDiv let-state="state" let-row="row" let-i="i">
      ...
      >
        @for (item of state.items; track item) {
          <div [svyContainerStyle]="item" [svyContainerLayout]="item.layout" class="svy-wrapper" [style]="item.model.visible === false ? { display: 'none' } : null" style="position:absolute">
            <!-- wrapper div -->
            <ng-template [ngTemplateOutlet]="getRowItemTemplate(item)" [ngTemplateOutletContext]="{ state: getRowItemState(item, row, i), callback: this, row: row, i: i }"></ng-template>
            <!-- component  -->
          </div>
...
export class ListFormComponent extends ServoyBaseComponent<HTMLDivElement> implements AfterViewInit, OnDestroy, IApiExecutor {
  ...
  ngOnChanges(changes: SimpleChanges) {
    ...
    if (containedForm && containedForm.childElements) {
      ...
      containedForm.childElements.forEach((elem) => {
        if (elem.specName === 'servoycore-formcomponent')
          // TODO this used elem.type before which was always camel-case; does this mean that this if should be removed instead of making it work?
        ...
        if (event.viewportRowsUpdated) {
          ...
          if (event.viewPortStartIndexChanged) {
            ...
            if (shiftedPageDelta !== 0) {
              ...
              if (loadExtraCorrected > 0 /*so shift right*/ && wantedVPStartIndex + wantedVPSize > serverSize) loadExtraCorrected -= wantedVPStartIndex + wantedVPSize - serverSize;
          ...
          const foundset = this._foundset()!;
          if (
            event.viewPortSizeChanged &&
            this._foundset()!.serverSize > 0 &&
            this.page * this.numberOfCells >= this._foundset()!.serverSize &&
            foundset.viewPort.size === 0 &&
  ...
  getRowItemState(item: StructureCache | FormComponentCache | ComponentCache, row: ViewPortRow, rowIndex: number): Cell | StructureCache | FormComponentCache {
    ...
    if (!cm.triggerNgOnChangeWithSameRefDueToSmartPropertyUpdate) {
      // declare it only once for all rows in ComponentValue - so it can be used by component_converter.ts
      cm.triggerNgOnChangeWithSameRefDueToSmartPropertyUpdate = (propertiesChangedButNotByRef: { propertyName: string; newPropertyValue: any }[], relativeRowIndex: number) => {
        const triggerNgOnChangeForThisComponentInGivenRow = (rowObject: Record<string, ServoyBaseComponent<any>>, componentModel: any) => {
          ...
          if (ui) {
            ...
            propertiesChangedButNotByRef.forEach((propertyChangedButNotByRef) => {
              const newValue =
                componentModel && componentModel[propertyChangedButNotByRef.propertyName] !== undefined
                  ? componentModel[propertyChangedButNotByRef.propertyName]
                  : propertyChangedButNotByRef.newPropertyValue;
        ...
        let relativeStartIndex = relativeRowIndex,
          relativeStopIndex = relativeRowIndex + 1;
      ...
      if (!row) {
        this.log.warn(
          'calling api ' +
            ...
            selectedIndex +
            '  is not in the view' +
            this._foundset()!.viewPort +
            ' fallback to the nearest visible row',
        );
        ...
        if (instanceOfApiExecutor(uiComp)) {
          ...
          this.log.error(
            'trying to call api: ' +
              ...
              path +
              ', but comp: ' +
              (uiComp == null ? ' is not found' : uiComp.name + ' doesnt implement IApiExecutor'),
          );
        }
      } else {
        AbstractFormComponent.doCallApiOnComponent(uiComp, this.typesRegistry.getComponentSpecification(compModel.type), apiName, args, this.converterService, this.log, componentName);
      }
    } else {
    ...
    if (!this.useScrolling) {
      ...
      if (foundset.viewPort.startIndex !== startIndex) {
        ...
        if (this.numberOfCells > foundset.viewPort.rows.length && foundset.viewPort.startIndex + foundset.viewPort.size < foundset.serverSize) {
          this.waitingForLoad = true;
          foundset
            .loadExtraRecordsAsync(Math.min(this.numberOfCells - foundset.viewPort.rows.length, foundset.serverSize - foundset.viewPort.startIndex - foundset.viewPort.size))
...
class Cell {
  ...
  constructor(
    public readonly state: IChildComponentPropertyValue,
    public readonly handlers: any,
    public readonly rowId: any,
...
class ListFormComponentServoyApi extends ServoyApi {
  ...
  apply(propertyName: string, value: any) {
    this.cell.state.sendChanges(propertyName, value, this.cell.model[propertyName], this.cell.rowId, true); // this will automatically change selection to this row server-side as well if needed
    this.fireOnSelectionChangeIfNeeded();
  }
