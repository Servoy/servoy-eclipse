...
export class ServoyExtraCollapse extends ServoyBaseComponent<HTMLDivElement>{
    ...
    @Input() collapsibles;
	@Input() accordionMode;
	@Input() expandedIndices;

	//handlers
	@Input() onCollapsibleShown;
	@Input() onCollapsibleHidden;
	@Input() onHeaderClicked;
	@Input() onCardClicked;
    ...
    svyOnChanges(changes: SimpleChanges) {
		console.log('svyOnChanges - called');
		console.log(changes);
	...
	onClick(e) {
        ...
        const collapsibleIndex = e.target.closest('.svy-collapse-collapsible').getAttribute('id').split('-')[1];
	...
	onCardClick(e, cardIndex, collapsibleIndex) {
	...
	getCardFormIfVisible(collapse, card) {
	 ...
	 */
	private toggleCardVisibility(cardsArray, state) {
		return Promise.all(cardsArray.map((card) => {
			this.toggleFormVisibility(card, state);
		}));
	}

	private toggleFormVisibility(card, state) {
		if (card.form) {
			if (state === false) {
				return this.servoyApi.formWillShow(card.form, card.relationName);
			} else {
				return this.servoyApi.hideForm(card.form, card.relationName);
			}
		} else {
			return true;
		}
	 ...
	 */
	private getFormState(form, collapsibleOrCard) {
		this.servoyApi.formWillShow(form, collapsibleOrCard.relationName).then(() => {
