...
export class ServicesService {
  ...
  public callServiceApi(serviceCall: ApiCallFromServer, previousResponseValue: any): any {
    ...
    if (serviceInstance && serviceInstance[serviceCall.call]) {
      const serviceCallSpec = serviceSpec?.getApiFunction(serviceCall.call);

      if (serviceCall.args)
        for (let argNo = 0; argNo < serviceCall.args.length; argNo++) {
          serviceCall.args[argNo] = this.converterService.convertFromServerToClient(
            serviceCall.args[argNo],
            serviceCallSpec?.getArgumentType(argNo),
            undefined,
            undefined!,
            undefined!,
      ...
      if (serviceCallSpec?.shouldReturnValue || serviceCallSpec === undefined) {
        ...
        return Promise.resolve(serviceInstance[serviceCall.call].apply(serviceInstance, serviceCall.args)).then(
          (ret) => this.converterService.convertFromClientToServer(ret, serviceCallSpec?.returnType, undefined, PushToServerUtils.PROPERTY_CONTEXT_FOR_OUTGOING_ARGS_AND_RETURN_VALUES)[0],
          (reason) => {
            // error
  ...
  public updateServiceScopes(services: any) {
    for (const servicename of Object.keys(services)) {
      ...
      if (service) {
        ...
        const propertyContextCreator: IPropertyContextCreator = PushToServerUtils.newRootPropertyContextCreator((propertyName: string) => service[propertyName], serviceSpec);
        try {
          ...
          for (const propertyName of Object.keys(serviceData)) {
            ...
            service[propertyName] = this.converterService.convertFromServerToClient(
              serviceData[propertyName],
              staticPropertyType /*update from server; old dynamic type does not matter*/,
              service[propertyName],
              dynamicPropertyTypesForThisService,
              propertyName,
            ...
            if (newPropertyType && (service[propertyName] !== oldValue || oldPropertyType !== newPropertyType)) this.setChangeListenerIfSmartProperty(service[propertyName], servicename, propertyName);
  ...
  public sendServiceChangesWithValue(serviceName: string, propertyName: string, propertyValue: any, oldPropertValue: any) {
    ...
    if (!propertyType) {
      // try to see if this prop. had a dynamic type then
  ...
  private getServiceDynamicClientSideTypes(serviceName: string) {
    ...
    if (!dynamicTypesForService) this.serviceDynamicClientSideTypes[serviceName] = dynamicTypesForService = {};

    return dynamicTypesForService;
  }
}

interface ApiCallFromServer {
  name: string;
  call: string;
  args: any[];
  pre_data_service_call?: boolean;
}
