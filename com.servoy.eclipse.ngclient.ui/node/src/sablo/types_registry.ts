...
export class TypesRegistry implements ITypesRegistryForTypeFactories, ITypesRegistryForSabloConverters {
  ...
  registerGlobalType(typeName: string, theType: IType<any>, onlyIfNotAlreadyRegistered?: boolean) {
    ...
    if (this.types[typeName]) {
      this.logger.debug(
        '[TypesRegistry] registerGlobalType - a global type with the same name (' +
          typeName +
          ') was already previously registered. Old: ' +
          this.types[typeName].constructor['name'] +
          ', New: ' +
          theType.constructor['name'] +
          '. The old one will be' +
          (onlyIfNotAlreadyRegistered ? 'kept' : ' discarded.'),
      );
  ...
  processTypeFromServer(typeFromServer: ITypeFromServer, webObjectSpecName: string): IType<any> | undefined {
    if (typeof typeFromServer == 'string') {
      ...
      if (!typeFactory) {
        this.logger.error(
          "[TypeRegistry] trying to process factory type into actual specific type for a factory type with name '" + factoryTypeFromServer[0] + "' but no such factory is registered...",
        );
        return undefined;
      } else {
  ...
  addComponentClientSideSpecs(componentSpecificationsFromServer: IWebObjectTypesFromServer) {
    if (this.logger.logLevel >= LogLevel.SPAM) this.logger.spam('[typesRegistry] Adding component specifications for: ' + JSON.stringify(componentSpecificationsFromServer, undefined, 2));
...
export class RootPropertyContextCreator implements IPropertyContextCreator {
  constructor(
    private readonly getProperty: IPropertyContextGetterMethod,
    private readonly webObjectSpec: IWebObjectSpecification,
  ) {}

  withPushToServerFor(rootPropertyName: string): PropertyContext {
    return new PropertyContext(this.getProperty, this.webObjectSpec ? this.webObjectSpec.getPropertyPushToServer(rootPropertyName) : PushToServerEnum.REJECT, true); // getPropertyPushToServer not getPropertyDeclaredPushToServer
  }
}
...
export class ChildPropertyContextCreator implements IPropertyContextCreator {
  ...
  withPushToServerFor(childPropertyName: string): PropertyContext {
    return new PropertyContext(
      this.getProperty,
      PushToServerUtils.combineWithChildStatic(this.computedParentPushToServer, this.propertyDescriptions[childPropertyName]?.getPropertyDeclaredPushToServer()),
      this.isInsideModel,
    ); // getPropertyDeclaredPushToServer not getPropertyPushToServer
  }
}

export class PropertyContext implements IPropertyContext {
  constructor(
    public readonly getProperty: IPropertyContextGetterMethod,
...
export class PushToServerUtils {
  ...
  public static combineWithChildStatic(parentComputedPushToServer: PushToServerEnum, childDeclaredPushToServer: PushToServerEnum | undefined | null) {
    ...
    if (parentComputedPushToServer === PushToServerEnum.REJECT || childDeclaredPushToServer === PushToServerEnum.REJECT) computed = PushToServerEnum.REJECT;
      ...
      if (childDeclaredPushToServer == null)
        computed = parentComputedPushToServer; // parent cannot be undefined
  ...
  public static newChildPropertyContextCreator(
    getProperty: IPropertyContextGetterMethod,
    propertyDescriptions: Record<string, IPropertyDescription>,
    computedParentPushToServer: PushToServerEnum,
    isInsideModel: boolean,
  ): IPropertyContextCreator {
 ...
 */
export interface IPropertyDescriptionFromServerWithMultipleEntries {
  t?: ITypeFromServer;
  s: PushToServerEnumServerValue;
}
