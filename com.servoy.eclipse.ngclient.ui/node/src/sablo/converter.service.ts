...
export class ConverterService<T> {
  ...
  public convertFromServerToClient(
    serverSentData: unknown,
    ...
    if (typeOfData) {
      ...
      if (lookedUpType) {
        ...
        // needed type not found - will not convert
        this.log.error(
          'no such type was registered (s->c varying or default type conversion) for: ' +
   ...
   */
  public convertFromClientToServer(newClientData: any, typeOfData: IType<any> | undefined, oldClientData: any, propertyContext: IPropertyContext): [any, any] {
    ...
    if (typeOfData) ret = typeOfData.fromClientToServer(newClientData, oldClientData, propertyContext);
      ...
      if (this.lookedUpObjectType) ret = this.lookedUpObjectType.fromClientToServer(newClientData, oldClientData, propertyContext);
        ...
        // 'object' type not found?! it should always be there - no default conversion can be done...
    ...
    if (!ret)
      ret = [null, newClientData]; // keep value unchanged, null JSON to send to server
  ...
  public getEventArgs(args: any[] | IArguments, eventName: string, handlerSpecification: IWebObjectFunction) {
    ...
    for (let i = 0; i < args.length; i++) {
      ...
      if (
        ...
        (arg?.constructor ? (window as Record<string, any>)[arg?.constructor?.name]?.['prototype'] instanceof MouseEvent : false) ||
        (arg?.constructor ? (window as Record<string, any>)[arg?.constructor?.name]?.['prototype'] instanceof KeyboardEvent : false)
      ) {
        const $event = arg;
...
export function isChanged(now: any, prev: any) {
  ...
  if (now && prev) {
    ...
    if (now instanceof Object && prev instanceof Object) {
      // this is true both for objects and for arrays
...
export const instanceOfChangeAwareValue = (obj: any): obj is IChangeAwareValue => obj != null && obj.getInternalState instanceof Function;
...
export const instanceOfUIDestroyAwareValue = (obj: any): obj is IUIDestroyAwareValue => obj != null && obj.uiDestroyed instanceof Function;
...
export const instanceOfIInternalReferenceBetweenPropertyImpls = (obj: any): obj is IInternalReferenceBetweenPropertyImpls => obj != null && obj.internalReferenceBetweenPropertyImpls;
...
export class SubpropertyChangeByReferenceHandler {
  ...
  public setPropertyAndHandleChanges(parentValue: any, propertyName: number | string, value: any, doNotPushNow?: boolean) {
    ...
    this.setChangeListenerToSubValueIfNeeded(
      parentValue[propertyName] /* markIfChanged call above might have upated it again due to clientToServer conversion for array/custom
