...
export class WebsocketService {
  ...
  public connect(context: string, args: string[], queryArgs?: Record<string, unknown>, websocketUri?: string, mobile = false): WebsocketSession {
    ...
    this.ngZone.runOutsideAngular(() => {
      ...
      const websocket = mobile
        ? new MobileBridge(this.windowRef)
        : new ReconnectingWebSocket(
            (reconnectAttempt?: boolean) =>
              this.generateURL(
                this.connectionArguments['context'],
                this.connectionArguments['args'],
                this.connectionArguments['queryArgs'],
                this.connectionArguments['websocketUri'],
                reconnectAttempt,
              ),
            this.logFactory,
          );
  ...
  private handleMessage(message: any) {
    ...
    try {
      ...
      if (obj && obj.smsgid) {
        ...
        Promise.resolve(responseValue).then(
          (ret) => {
            // success
...
export const wrapPromiseToPropagateCustomRequestInfoInternal = (originalPromise: RequestInfoPromise<any>, spawnedPromise: RequestInfoPromise<any>): RequestInfoPromise<any> => {
