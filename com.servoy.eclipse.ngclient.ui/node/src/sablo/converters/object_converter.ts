...
export class ObjectType implements IType<any> {
  ...
  constructor(
    private readonly typesRegistry: ITypesRegistryForSabloConverters,
  ...
  fromServerToClient(serverJSONValue: any, currentClientValue: any, propertyContext: IPropertyContext): any {
    ...
    if (serverJSONValue instanceof Object) {
      ...
      if (serverJSONValue[ConverterService.CONVERSION_CL_SIDE_TYPE_KEY] !== undefined) {
        ...
        serverJSONValue = this.converterService.convertFromServerToClient(serverJSONValue, undefined, currentClientValue, undefined!, undefined!, propertyContext);
      } else {
        // see which nested sub-property/sub-element
        for (const i in serverJSONValue)
          // works for both arrays (indexes) and objects (keys) in JS
          if (serverJSONValue[i] instanceof Object && serverJSONValue[i][ConverterService.CONVERSION_CL_SIDE_TYPE_KEY] !== undefined)
            serverJSONValue[i] = this.converterService.convertFromServerToClient(
              serverJSONValue[i],
              undefined,
              currentClientValue ? currentClientValue[i] : undefined,
              undefined!,
              undefined!,
              propertyContext,
            );
  ...
  private fromClientToServerInternal(
    newClientData: any,
    oldClientData: any,
    propertyContext: IPropertyContext,
    alreadyProcessedNestingValues: Set<unknown>,
    ...
    if (newClientData instanceof Date) {
      ...
      if (dateType) {
        ...
        retVal[ConverterService.VALUE_KEY] = this.converterService.convertFromClientToServer(newClientData, dateType, oldClientData, propertyContext)[0];
      }
      ...
      if (alreadyProcessedNestingValues.has(newClientData)) {
        if (!cyclicDepError)
          this.log.error(
            new Error(
              "fromClientToServer: plain 'object' typed data cannot be sent to server (recursive same references will be changed to null) when it contains circular references. One of the circular refs is: " +
                newClientData,
            ),
          );
        retVal = null;
