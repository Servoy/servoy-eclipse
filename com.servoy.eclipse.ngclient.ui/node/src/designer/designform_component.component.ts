...
@Component({
  ...
  template: `
    @if (formCache.absolute) {
      <div [ngStyle]="getAbsoluteFormStyle()" class="svy-form" [ngClass]="formClasses">
        ...
        @for (part of formCache.parts; track part) {
          <div [svyContainerStyle]="part" [svyContainerLayout]="part.layout" [svyContainerClasses]="part.classes"><!-- part div --></div>
        }
        @for (item of formCache.partComponentsCache; track item) {
          <div
            [svyContainerStyle]="item"
            [svyContainerLayout]="item.layout"
            class="svy-wrapper"
            [ngClass]="{ invisible_element: item.model.svyVisible === false, inherited_element: item.model.svyInheritedElement }"
    ...
    @if (!formCache.absolute && name !== 'VariantsForm') {
      <div class="svy-form svy-respform svy-overflow-auto" [ngClass]="formClasses">
        <!-- main container div -->
        @if (draggedElementItem) {
          <div [svyContainerStyle]="draggedElementItem" [svyContainerLayout]="draggedElementItem.layout" class="svy-wrapper" style="position:absolute" id="svy_draggedelement">
            <ng-template [ngTemplateOutlet]="getTemplate(draggedElementItem)" [ngTemplateOutletContext]="{ state: draggedElementItem, callback: this }"></ng-template>
    ...
    @if (!formCache.absolute && name === 'VariantsForm') {
      <div class="svy-form svy-respform svy-overflow-auto" [ngClass]="formClasses" (keyup)="onVariantKeyup($event)">
        <!-- main container div -->
        @for (item of formCache.mainStructure?.items; track item) {
    ...
    <ng-template #formComponentAbsoluteDiv let-state="state">
      <div
        [ngClass]="{ invisible_element: state.model.svyVisible === false }"
      ...
      >
        @for (item of state.items; track item) {
          <div [svyContainerStyle]="item" [svyContainerLayout]="item.layout" class="svy-wrapper" [ngClass]="{ invisible_element: item.model.svyVisible === false }" style="position:absolute">
            <!-- wrapper div -->
            <ng-template [ngTemplateOutlet]="getTemplate(item)" [ngTemplateOutletContext]="{ state: item, callback: this }"></ng-template>
            <!-- component  -->
          </div>
    ...
    <ng-template #formComponentResponsiveDiv let-state="state">
      <servoycore-formcomponent-responsive-container
        [svyContainerStyle]="state.formComponentProperties"
        [svyContainerAttributes]="state.formComponentProperties.attributes"
        [ngClass]="{ invisible_element: state.model.svyVisible === false }"
        [items]="state.items"
        [class]="state.model.styleClass"
        [formComponent]="this"
      ></servoycore-formcomponent-responsive-container>
      @if (!state.items || !state.items.length) {
        <div>FormComponentContainer, select a form.</div>
    ...
    <ng-template #servoycoreListformcomponent let-callback="callback" let-state="state"
      ><servoycore-listformcomponent
        [containedForm]="state.model.containedForm"
        [foundset]="state.model.foundset"
        [pageLayout]="state.model.pageLayout"
        [readOnly]="state.model.readOnly"
        [responsivePageSize]="state.model.responsivePageSize"
        [rowStyleClass]="state.model.rowStyleClass"
        ...
        [onSelectionChanged]="callback.getHandler(state, 'onSelectionChanged')"
...
export class DesignFormComponent extends AbstractFormComponent implements OnDestroy, OnChanges, IFormComponent {
    ...
    this.windowRefService.nativeWindow.addEventListener('message', (event) => {
      if (event.data.id === 'createElement') {
        ...
        if (event.data.type === 'layout') {
          ...
          this.insertedClone = new ComponentCache(event.data.model.tagname, event.data.name, undefined!, [], model_inserted, typesRegistry).initForDesigner(event.data.model); // TODO only in responsive
        }
      ...
      if (event.data.id === 'destroyVariants') {
        if (this.insertedVariants) {
          ...
          if (this.insertedVariants) {
            for (const insertedVariant of this.insertedVariants) {
              //need to remove previous existing variants
      ...
      if (event.data.id === 'createDraggedComponent') {
        ...
        if (this.insertedClone) {
          if (event.data.dragCopy) {
            ...
            this.insertedClone = new StructureCache(
              (this.insertedClone as StructureCache).tagname,
              (this.insertedClone as StructureCache).classes,
              (this.insertedClone as StructureCache).attributes,
              (this.insertedClone as StructureCache).items,
            );
            parent.addChild(this.insertedClone);
          }
          this.draggedElementItem = new StructureCache(
            (this.insertedClone as StructureCache).tagname,
            (this.insertedClone as StructureCache).classes,
            Object.assign({}, (this.insertedClone as StructureCache).attributes),
            (this.insertedClone as StructureCache).items,
          );
          ...
          if (event.data.dragCopy) {
            const parent = this.insertedClone.parent;
            this.insertedClone = new ComponentCache(
              (this.insertedClone as ComponentCache).name + 'clone',
              (this.insertedClone as ComponentCache).specName,
              (this.insertedClone as ComponentCache).type,
              (this.insertedClone as ComponentCache).handlers,
              (this.insertedClone as ComponentCache).layout,
              this.typesRegistry,
            ).initForDesigner(oldModel);

            parent.addChild(this.insertedClone);
          }

          this.draggedElementItem = new ComponentCache(
            'dragged_element',
            (this.insertedClone as ComponentCache).specName,
            (this.insertedClone as ComponentCache).type,
            (this.insertedClone as ComponentCache).handlers,
  ...
  sendVariantSizes() {
    ...
    this.windowRefService.nativeWindow.parent.parent.parent.postMessage({ id: 'resizePopover', formWidth, formHeight }, '*');
  }
