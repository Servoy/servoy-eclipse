...
import { PersistIdentifier } from './persistidentifier';
...
export class EditorContentService {
  ...
  updateFormData(updates: string) {
      ...
      for (const index of Object.keys(data.updatedFormComponentsDesignId)) {
        ...
        if (fc) {
          fc.items.forEach((item) => {
            if (item instanceof ComponentCache) formCache.removeComponent(item.name);
    ...
    if (data.ng2containers) {
      // this can contain layout containers only directly from the root form; not nested inside form component components
    ...
    if (data.ng2components) {
          ...
          if (!handledArrayIndexes[indexInArray]) {
            ...
            if (component) {
              ...
              if (component instanceof ComponentCache && component.parent) {
                // .parent can only be a layout container (so responsive)
              ...
              if (handled) {
                // so if it's not supposed to have a parent or if the parent was found we can proceed
                redrawDecorators = true;
                ...
                for (const propName of Object.keys(rawModelProperties)) {
                  elem.model[propName] = this.converterService.convertFromServerToClient(
                    rawModelProperties[propName],
                ...
                if (elem.specName === 'servoycore-formcomponent' || elem.specName === 'servoycore-listformcomponent') {
                  ...
                  containers?.forEach((elem: any) => {
                    ...
                    if (parentUUID && parentUUID !== fcc.name) {
                      ...
                      if (parent) {
                        ...
                        this.logger.error(
                          this.logger.buildMessage(
                            () => 'Cannot find parent layout container (' + parentUUID + ') for a layout container (' + container.id + ') when updates were received in editor. Incorrect order?',
                          ),
                        );
                    } else {
                  ...
                  const comp = new ComponentCache(elem.name, elem.specName, elem.elType, elem.handlers, elem.position ? elem.position : elem.model.cssPosition, this.typesRegistry).initForDesigner(
                    elem.model,
                  );
                  formCache.add(comp);
                  if (parentUUID) {
      ...
      if (numberOfParentsNotFound > 0)
        this.logger.error(
          this.logger.buildMessage(
            () => 'A number of ' + numberOfParentsNotFound + ' components, layout containers or form component components could not find their parents when updates were received in the editor.',
          ),
        );
          ...
          if (data.updatedFormComponentsDesignId) {
            ...
            if (data.updatedFormComponentsDesignId.indexOf(elem.name) !== -1) {
              ...
              data.formComponentsComponents?.forEach((child: string) => {
                // so "child" is actually a design id here; see PersistIdentifier java class
    ...
    for (const container of orphanLayoutContainers) {
      // I think these are only from the main form, not nested inside other form component components
      const parentUUID = data.childParentMap[container.id!].uuid;
      if (parentUUID) {
        const parent = formCache.getLayoutContainer(parentUUID);
        if (parent) {
  ...
  updateComponentProperties(component: IComponentCache, elem: ServerElement): boolean {
    ...
    for (const propName of Object.keys(elem.model)) {
      const value = this.converterService.convertFromServerToClient(
        elem.model[propName],
        componentSpec?.getPropertyType(propName),
        component.model[propName],
        componentDynamicTypesHolder,
        propName,
        propertyContextCreator.withPushToServerFor(propName),
      ) as CSSPosition & Position;
      // as we are in designer here, we don't listen for any potential change aware values after conversions (see FormService.handleComponentModelConversionsAndChangeListeners(...))
      if (
        ...
        (propName === 'cssPosition' &&
          (component.model[propName].top !== value.top ||
            component.model[propName].bottom !== value.bottom ||
            component.model[propName].left !== value.left ||
            component.model[propName].right !== value.right ||
            component.model[propName].width !== value.width ||
            component.model[propName].height !== value.height))
      ) {
        redrawDecorators = true;
      }
  ...
  private isElementCompatibleWithParent(parentFirstItem: StructureCache | ComponentCache | FormComponentCache, newItemName: string): boolean {
    const [parentItemUUID] = JSON.parse(parentFirstItem.name);
    const [newItemUUID] = JSON.parse(newItemName);
