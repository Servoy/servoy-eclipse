...
| peerDependencies | ? | Library package.json still has peerDependencies? |
| .npmrc legacy-peer-deps | ? | .npmrc contains legacy-peer-deps=true? |
```
...
13. **peerDependencies**: check `projects/*/package.json` (the library package.json) for `peerDependencies`
14. **.npmrc**: check if `.npmrc` contains `legacy-peer-deps=true`

Present the table, then ask the user which phase to start with (or suggest the logical next phase).

---

## Phase 0.5 — Remove peerDependencies and legacy-peer-deps

Servoy web packages are always installed into the TiNG host application which already provides Angular, RxJS, and `@servoy/public`. The `peerDependencies` in the library `package.json` (the one inside `projects/*/`) served as documentation of what the host provides, but they cause real problems:

- **npm strict mode** (default since npm 7) errors on unmet peer deps, forcing `--legacy-peer-deps` everywhere
- **bun** does not support `--legacy-peer-deps` at all
- **Version drift**: each package independently declares Angular version ranges that must be kept in sync — a maintenance burden with no benefit
- **Cross-package conflicts**: when two packages declare different `^` ranges for the same peer, npm may install duplicate copies, breaking Angular's compilation

### What to do

1. **Remove the entire `peerDependencies` block** from the library `package.json` (the one in `projects/<name>/package.json`). Keep `dependencies` — those are real runtime dependencies that npm must install.

2. **Also remove `peerDependencies` from the `dist/` copy** if one exists (the built output). This is usually regenerated by `ng build`, but stale dist copies may linger.

3. **Remove `legacy-peer-deps=true` from `.npmrc`** (root of the Angular workspace). If `.npmrc` has other settings (like `stack-size`), keep those — only remove the `legacy-peer-deps` line. If `legacy-peer-deps=true` was the only line, delete the file entirely.

4. **Remove `allowedNonPeerDependencies`** from `ng-package.json` — this option is only relevant when peerDependencies exist.

5. **Verify**: run `npm install` (without `--legacy-peer-deps`) and confirm it succeeds cleanly.

### Why this is safe

These packages are never installed as standalone npm packages from a registry by end users. They are:
- Built as Angular libraries (`ng build`)
- Packaged into `.zip` files for the Servoy Web Package Manager
- Installed into TiNG by the Servoy build system, which copies the compiled output into the app

The host application's `package.json` (TiNG's `node/package.json`) controls which versions of Angular, RxJS, etc. are used. The library's `peerDependencies` were only a declaration of what the host provides — removing them changes nothing about what actually gets installed.

Commit: `remove peerDependencies and legacy-peer-deps [ai]`
