...
/**
 * One JSON-RPC call, put straight through the servlet.
 *
 * <p>No socket is involved. What is being tested here is the server's own behaviour - who a token
 * belongs to, which rows a tenant sees, whether a changed solution is rescanned - and none of that
 * becomes more true for having travelled over TCP. It does become slower and less reliable, because
 * the test then has to wait for a port to open. {@link McpHttpIntegrationTest} takes the HTTP path
 * once, to prove the wiring; everything else comes through here.</p>
 *
...
public final class McpCall
{
	...
	/** The HTTP status the servlet set. */
	public int status()
	{
		return status;
	}
	 ...
	 */
	public static McpCall toolsList(String solutionName) throws Exception
	{
		return post(solutionName, "{\"jsonrpc\":\"2.0\",\"id\":1,\"method\":\"tools/list\"}", null);
	}
	 ...
	 */
	public static McpCall callTool(String solutionName, String toolName, String argumentsJson, String token) throws Exception
	{
		String payload = "{\"jsonrpc\":\"2.0\",\"id\":2,\"method\":\"tools/call\",\"params\":{\"name\":\"" +
			toolName + "\",\"arguments\":" + (argumentsJson == null ? "{}" : argumentsJson) + "}}";

		return post(solutionName, payload, token);
	}

	/**
	 * Sends a raw JSON-RPC payload to one solution's endpoint.
	 */
	public static McpCall post(String solutionName, String payload, String token) throws Exception
	{
		...
		HttpServletRequest request = request("/mcp/" + solutionName, payload, headers);
		HttpServletResponse response = response(written, status);

		new McpServlet().service(request, response);

		return new McpCall(written.toString(), status[0]);
	}

	private static HttpServletRequest request(final String pathInfo, final String payload, final Map<String, String> headers)
	{
		...
		InvocationHandler handler = new InvocationHandler()
		{
			...
			public Object invoke(Object proxy, Method method, Object[] arguments) throws Throwable
			{
				switch (method.getName())
				{
					...
					case "toString" :
						return "McpCall request " + pathInfo;
					default :
