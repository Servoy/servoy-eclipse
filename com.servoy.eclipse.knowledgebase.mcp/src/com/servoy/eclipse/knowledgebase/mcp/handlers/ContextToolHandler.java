 ...
 */
public class ContextToolHandler implements IToolHandler
{
	@Override
	...
	private McpSchema.CallToolResult handleGetContext(McpSyncServerExchange exchange, McpSchema.CallToolRequest request)
	{
		...
		{
			return errorResult("Error getting context: " + e.getMessage());
		}
	}
	...
	private McpSchema.CallToolResult handleSetContext(McpSyncServerExchange exchange, McpSchema.CallToolRequest request)
	{
		try
		{
			...
			if (targetContext == null || targetContext.trim().isEmpty())
			{
				return errorResult("'context' parameter is required");
			}
		...
		{
			return errorResult("Error setting context: " + e.getMessage());
		}
	}
	...
	{
		return McpSchema.CallToolResult.builder().content(List.of(new TextContent(message))).build();
	}

	private McpSchema.CallToolResult errorResult(String message)
	{
		return McpSchema.CallToolResult.builder().content(List.of(new TextContent("Error: " + message))).isError(true).build();
	}
}
