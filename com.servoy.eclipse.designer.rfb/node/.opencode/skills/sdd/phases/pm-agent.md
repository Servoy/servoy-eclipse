...
| UI/UX | Is the visual behaviour described (drag interactions, overlays, layout)? |
| Layout types | Does this affect absolute layout, responsive layout, or both? |
| WebSocket API | Are new backend calls needed? What data flows? |
| Dependencies | Known dependencies on `@servoy/sablo`, `@servoy/public`, `@servoy/designer`? |
...
Use search tools (`grep`, `glob`, file reading) to understand the relevant parts
of the Angular codebase:

- Find existing implementations of similar features in `src/designer/`
- Understand the component structure and where new code should live
- Check `src/designer/services/` for relevant services
- Look at how similar UI interactions are handled (drag, selection, overlays)
- Check cross-project dependencies (`@servoy/sablo`, `@servoy/public`, `@servoy/designer`)

Key areas to investigate:
- `src/designer/designer.component.ts` — root component template, overall structure
- `src/designer/designer.module.ts` — module declarations, imports
- `src/designer/services/editorsession.service.ts` — WebSocket communication
- `src/designer/services/editorcontent.service.ts` — iframe/DOM abstraction
...
<Describe the proposed design. For UI features, describe:
- Visual behaviour (what the user sees)
- Interaction model (mouse/keyboard events)
- Layout type handling (absolute vs responsive)
- WebSocket calls to Eclipse backend (if any)>

### 3.2 <Second design area>
...

## 4. Implementation plan
<Ordered list of the concrete changes needed — files to create/modify,
components to add, services to update, etc.>
...
```
docs/SVY-21129-dynamic-guides-resize.spec.md
```
