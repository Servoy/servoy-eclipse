...
export class PaletteComponent implements ISupportAutoscroll {
  ...
  constructor(
    ...
    protected designerUtilsService: DesignerUtilsService,
    private editorContentService: EditorContentService,
    private windowRef: WindowRefService
  ) {
    let layoutType: string;
    if (urlParser.isAbsoluteFormLayout()) layoutType = 'Absolute-Layout';
    else layoutType = 'Responsive-Layout';
    this.activeIds = [];
    this.http
      .get(
        '/designer/palette?layout=' +
          layoutType +
          '&formName=' +
          this.urlParser.getFormName()
      )
      .subscribe((got: Array<Package>) => {
        ...
        for (let i = 0; i < packages.length; i++) {
          packages[i].id = ('svy_' + packages[i].packageName)
            .replace(/[|&;$%@"<>()+,]/g, '')
            .replace(/\s+/g, '_');
            ...
            for (let j = 0; j < packages[i].components.length; j++) {
              if (propertyValues && packages[i].components[j].properties) {
                ...
                if (
                  ...
                  packages[i].components[j].name == 'servoycore-formcomponent'
                ) {
  ...
  onMouseDown(
    ...
    Array.from(this.dragItem.paletteItemBeingDragged.children).forEach(
      (child) => {
        if (
          child.tagName == 'UL' ||
          child.nodeName == 'DESIGNER-VARIANTSCONTENT'
        ) {
    ...
    if (!ghost) {
      ...
      this.editorContentService.sendMessageToIframe({
        id: 'createElement',
        name: this.convertToJSName(elementName),
        model: model,
        type: componentType,
        attributes: attributes,
        children: children,
      });
      this.autoscrollElementClientBounds =
        this.designerUtilsService.getAutoscrollElementClientBounds();
    }
  }
  ...
  onMouseUp = (event: MouseEvent) => {
    if (this.dragItem.paletteItemBeingDragged) {
      ...
      component.x =
        component.x - this.editorContentService.getLeftPositionIframe();
      component.y =
        component.y - this.editorContentService.getTopPositionIframe();
      ...
      if (this.urlParser.isAbsoluteFormLayout()) {
        if (this.canDrop.dropAllowed && this.canDrop.dropTarget) {
          component.dropTargetUUID =
            this.canDrop.dropTarget.getAttribute('svy-id');
        }
      } else {
        if (this.canDrop.dropAllowed) {
          ...
          if (this.canDrop.beforeChild) {
            component.rightSibling =
              this.canDrop.beforeChild.getAttribute('svy-id');
          }
      ...
      if (this.dragItem.ghost) {
        ...
        const found = Array.from(elements).find((node) => {
          ...
          if (
            ...
            position.x + position.width >= component.x &&
            position.y <= component.y &&
            position.y + position.height >= component.y
          ) {
            const types = node.getAttribute('svy-types');
            if (
              types &&
              types.split(',').indexOf(this.dragItem.ghost.type) >= 0
            ) {
              return node;
            }
          }
  ...
  onMouseMove = (event: MouseEvent) => {
    if (
      event.pageX >= this.editorContentService.getLeftPositionIframe() &&
      event.pageY >= this.editorContentService.getTopPositionIframe() &&
      this.dragItem.paletteItemBeingDragged &&
      this.dragItem.contentItemBeingDragged
    ) {
      this.renderer.setStyle(
        this.dragItem.paletteItemBeingDragged,
        'opacity',
    ...
    if (this.dragItem.paletteItemBeingDragged) {
      ...
      if (this.dragItem.contentItemBeingDragged) {
        ...
        this.canDrop = this.designerUtilsService.getDropNode(
          this.urlParser.isAbsoluteFormLayout(),
          this.dragItem.componentType,
          this.dragItem.topContainer,
          this.dragItem.layoutName
            ? this.dragItem.packageName + '.' + this.dragItem.layoutName
            : this.dragItem.layoutName,
          event,
          this.dragItem.elementName
        );
        if (!this.canDrop.dropAllowed) {
        ...
        if (this.dragItem.contentItemBeingDragged) {
          if (this.canDrop.dropAllowed) {
            ...
            this.editorContentService.sendMessageToIframe({
              ...
              dropTarget: this.canDrop.dropTarget
                ? this.canDrop.dropTarget.getAttribute('svy-id')
                : null,
              insertBefore: this.canDrop.beforeChild
                ? this.canDrop.beforeChild.getAttribute('svy-id')
                : null,
            });
          }
          if (
            this.canDrop.dropAllowed &&
            (this.canDrop.dropTarget || !this.urlParser.isAbsoluteFormLayout())
          ) {
        ...
        if (!this.dragItem.ghost) {
          ...
          this.dragItem.contentItemBeingDragged =
            this.editorContentService.getContentElementById(
              'svy_draggedelement'
            );
          if (this.dragItem.contentItemBeingDragged) {
            this.renderer.setStyle(
      ...
      if (
        this.autoscrollElementClientBounds &&
        !this.autoscrollAreasEnabled &&
        !this.designerUtilsService.isInsideAutoscrollElementClientBounds(
          this.autoscrollElementClientBounds,
          event.clientX,
          event.clientY
        )
      ...
      if (packageAndName.length > 1) {
        ...
        for (let i = 1; i < packageAndName.length; i++) {
          if (packageAndName[1].length > 0)
            name +=
              packageAndName[i].charAt(0).toUpperCase() +
              packageAndName[i].slice(1);
        }
      }
    }
    return name;
