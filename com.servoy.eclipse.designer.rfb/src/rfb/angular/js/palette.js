    ...
    return {
      ...
      controller: function($scope, $element, $attrs, $http, $pluginRegistry) {
        ...
        var loadPalette = function() {
          $http({
            method: 'GET',
            url: '/designer/palette?layout=' + layoutType
          ...
          var mousemovecallback = $scope.registerDOMEvent("mousemove", "EDITOR", function(ev) {
            if (dragClone) {
              ...
              if (type == 'component' || type == "layout" || type == "template") {
                if (type == 'component') {
                  angularElement = $scope.getEditorContentRootScope().createComponent(
                    '<div style="border-style: dotted;"><' + tagName +
                    ' svy-model=\'model\' svy-api=\'api\' svy-handlers=\'handlers\' svy-autoapply-disabled=\'true\'/></div>',
                    model);
                } else {
