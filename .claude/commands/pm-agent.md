...
- `SVY-21080`
- `https://servoy-cloud.atlassian.net/browse/SVY-21080`

## Steps

### 1. Extract the issue key

Parse `$ARGUMENTS` to get the bare issue key (e.g. `SVY-21080`).

### 2. Read the Jira issue

Try `mcp__atlassian__jira_get_issue` with the issue key.
If that tool is unavailable, try `mcp__atlassian__get_issue`.
If no Atlassian MCP tools are available, use WebFetch on the full URL.
...
If **more than one** important area is missing or too vague, ask the user to
clarify those points before writing the spec. Wait for their answers. If only
minor things are missing, make a reasonable assumption and note it as an open
...
Create the file using the Write tool or `eclipse-coder_createFile`.
