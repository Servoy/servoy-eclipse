...
public class ColumnDetailsComposite extends Composite
{
	...
	public static final String[] DATE_FORMAT_VALUES = new String[] { "#%", //$NON-NLS-1$
	"\u00A4#.00",//first char is currency symbol //$NON-NLS-1$
	...
	"MM-dd-yyyy hh:mm", //$NON-NLS-1$
	"yyyy-MM-dd HH:mm:ss.S" };

	private DataBindingContext bindingContext;
	private final TreeSelectViewer foreignTypeTreeSelect;
	private final Combo defaultFormatCombo;
	...
	public ColumnDetailsComposite(Composite parent, int style)
	{
		...
		defaultFormatCombo = new Combo(this, SWT.BORDER);
		...
		suggestForeignTypeButton.setText("Suggest..."); //$NON-NLS-1$
		...
		//start group
			...
			//start parallel group
				...
				add(defaultFormatCombo, GroupLayout.PREFERRED_SIZE, 450, Short.MAX_VALUE).
		...
		groupLayout.setVerticalGroup(groupLayout.createSequentialGroup().addContainerGap().add(
			...
			groupLayout.createParallelGroup(GroupLayout.CENTER, false).add(defaultFormatLabel).add(defaultFormatCombo)).addPreferredGap(LayoutStyle.RELATED).add(
	...
	void initDataBindings(final Column c)
	{
		...
		try
		{
			...
			{
				this.defaultFormatCombo.setText("");
			}
		...
		IObservableValue defaultFormatTextObserveWidget = SWTObservables.observeSelection(defaultFormatCombo);
