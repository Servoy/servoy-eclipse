...
public class CalculationsComposite extends Composite
{
	...
	public CalculationsComposite(final TableEditor te, Composite parent, FlattenedSolution flattenedSolution, int style)
	{
					...
					try
					{
						...
						if (solution != null)
						{
							...
							while (calc != null)
							{
								calcName = orgName + i;
								i++;
								calc = ServoyModelManager.getServoyModelManager().getServoyModel().getEditingFlattenedSolution(solution).getScriptCalculation(
									calcName, t);
							}
							ScriptCalculation s = solution.createNewScriptCalculation(nameValidator, t, calcName);
		...
		persistListener = new IPersistChangeListener()
		{
			public void persistChanges(Collection<IPersist> changes)
			{
				...
				{
					if (persist instanceof ScriptCalculation || persist instanceof TableNode)
					{
	...
	private void createTableColumns(Table table, final TableEditor te)
	{
		...
		treeViewer.setLabelProvider(new CalculationLabelProvider(table, new Color(treeViewer.getTree().getShell().getDisplay(), 255, 127, 0)));
	}

	protected void initDataBindings(Table t)
	{
		CalculationContentProvider columnViewContentProvider = new CalculationContentProvider(t);
