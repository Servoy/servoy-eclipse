...
/**
 * Action to load AI knowledge base (embeddings and rules) from a Servoy package
 * into the AI Bridge MCP server.
 *
 * Only enabled for packages that:
 * - Have Knowledge-Base: true in MANIFEST.MF
 * - Contain embeddings/embeddings.list
 * - Contain rules/rules.list
 *
 * @author Servoy
 */
public class LoadKnowledgeBaseAction extends Action implements ISelectionChangedListener
{
	private final SolutionExplorerView viewer;

	public LoadKnowledgeBaseAction(SolutionExplorerView viewer, String text)
	{
		super();
		this.viewer = viewer;
		setText(text);
	...
	public void run()
	{
		...
		if (node != null && (node.getRealObject() instanceof IPackageReader packageReader))
		{
			...
			if (ops != null)
			{
				try
				{
					ops.getClass().getMethod("loadKnowledgeBase", String.class).invoke(ops, packageName);
				}
				catch (Exception e)
				{
					ServoyLog.logError("Failed to load knowledge base from package: " + packageName, e);
				}
			}
			else
			{
