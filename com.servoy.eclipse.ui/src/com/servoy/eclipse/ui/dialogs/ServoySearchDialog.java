...
public class ServoySearchDialog extends FilteredItemsSelectionDialog
{
	...
	private static final String SHOW_FORMS = "ShowForms"; //$NON-NLS-1$
	private static final String SHOW_METHODS = "ShowMethods"; //$NON-NLS-1$
	private static final String SHOW_VARIABLES = "ShowVariables"; //$NON-NLS-1$
	private static final String SHOW_TABLES = "ShowTables"; //$NON-NLS-1$
	private static final String SHOW_CALCULATIONS = "ShowCalculations"; //$NON-NLS-1$
	private static final String SHOW_ELEMENTS = "ShowElements"; //$NON-NLS-1$
	private static final String SHOW_RELATIONS = "ShowRelations"; //$NON-NLS-1$
	private static final String SHOW_VALUELIST = "ShowValuelist"; //$NON-NLS-1$
	private static final String SHOW_MEDIA = "ShowValuelist"; //$NON-NLS-1$
	...
	public ServoySearchDialog(Shell shell)
	{
		...
		setSelectionHistory(new SelectionHistory()
		{
			...
			{
				String uuidString = memento.getString("UUID"); //$NON-NLS-1$
				if (uuidString == null)
				{
					String serverName = memento.getString("servername"); //$NON-NLS-1$
					String tableName = memento.getString("tablename"); //$NON-NLS-1$
			...
			protected void storeItemToMemento(Object item, IMemento memento)
			{
				...
				{
					memento.putString("UUID", ((IPersist)item).getUUID().toString()); //$NON-NLS-1$
				}
				else if (item instanceof Table)
				{
					Table table = (Table)item;
					memento.putString("servername", table.getServerName()); //$NON-NLS-1$
					memento.putString("tablename", table.getTableName()); //$NON-NLS-1$
				}
			}
		});
			...
			public String getText(Object element)
			{
				...
				if (element instanceof IPersist)
				{
					...
					if (element instanceof ScriptCalculation)
					{
						...
						name += " - " + node.getServerName() + '/' + node.getTableName(); //$NON-NLS-1$
					}
					else
					{
						name += " - " + parent.substring(1); //$NON-NLS-1$
					}
				}
				return name;
			}
	...
	public static class Table implements ISupportName
	{
		...
		{
			return tableName + " - " + serverName; //$NON-NLS-1$
		}
