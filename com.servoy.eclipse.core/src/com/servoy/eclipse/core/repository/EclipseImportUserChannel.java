...
public class EclipseImportUserChannel implements IXMLImportUserChannel
{
	...
	public int askRenameRootObjectAction(final String name, final int objectTypeId)
	{
		...
		final InputDialog nameDialog = new InputDialog(shell, objectType + " exists", objectType + " with name '" + name //$NON-NLS-1$
			+ "' already exists, specify new name:", "", new IInputValidator()
		{
	...
	public int askStyleAlreadyExistsAction(String name)
	{
		...
		final MessageDialog dialog = new MessageDialog(shell, "Style '" + name + "' already exists", null, "Style '" + name +
			"' exists in the workspace. Do you want to overwrite it or skip its import?", MessageDialog.WARNING, new String[] { "Overwrite", "Skip" }, 0);
		Display.getDefault().syncExec(new Runnable()
		{
			public void run()
			{
