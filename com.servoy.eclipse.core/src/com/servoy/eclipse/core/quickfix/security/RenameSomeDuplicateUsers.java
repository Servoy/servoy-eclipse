...
/**
 * Quick fix for renaming one of the duplicate users that appear in the users file.
 * 
 * @author acostescu
 */
public class RenameSomeDuplicateUsers extends AlterUserGroupSecFileQuickFix
{

	private static RenameSomeDuplicateUsers instance;

	public static RenameSomeDuplicateUsers getInstance()
	{
		if (instance == null)
		{
			instance = new RenameSomeDuplicateUsers();
		}
		return instance;
	}
		...
		if (duplicates.size() > 1)
		{
			ReturnValueRunnable runnable = new ReturnValueRunnable()
			{
				public void run()
				{
					...
					dialog.setMessage("Please select which user(s) should be renamed.");
					...
					if (choice == Window.OK)
					{
						...
						{
							ReturnValueRunnable r = new GetNewUserNameRunnable(users, "Rename user", ((User)user).name);
							UIUtils.runInUI(r, true);
							if (r.getReturnValue() != null)
							{
								((User)user).name = (String)r.getReturnValue();
							}
							else
							{
								// user pressed cancel
