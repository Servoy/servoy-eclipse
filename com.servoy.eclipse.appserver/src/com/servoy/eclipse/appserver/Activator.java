											...
											public IStatus runInWorkspace(final IProgressMonitor monitor) throws CoreException
											{
												try
												{
													...
													updatedToVersion[0] = ApplicationServerSingleton.get().updateAppServerFromSerclipse(
														new File(appServerDir).getParentFile(), version, ClientVersion.getReleaseNumber());
												}
												catch (Exception e)
												{
											...
											public void done(IJobChangeEvent event)
											{
												if (updatedToVersion[0] < ClientVersion.getReleaseNumber())
												{
													Display.getDefault().asyncExec(new Runnable()
													{
														public void run()
														{
															MessageDialog.openError(Display.getDefault().getActiveShell(), "Servoy update problem", //$NON-NLS-1$
												...
												else
												{
													Display.getDefault().asyncExec(new Runnable()
													{
														public void run()
														{
															MessageDialog.openInformation(Display.getDefault().getActiveShell(), "Servoy updated", //$NON-NLS-1$
																"Servoy ApplicationServer updated, please restart the developer"); //$NON-NLS-1$
														}
													});
												}
											}
