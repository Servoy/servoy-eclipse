...
class FormScope extends DefaultScope implements IProposalHolder
{
		...
		if (form != null)
		{
			...
			if (variable != null)
			{
					...
					(variable.getDataProviderType() == IColumnTypes.MEDIA || variable.getDataProviderType() == IColumnTypes.TEXT))
				{
					...
					if (!((String)initValue).startsWith("\"") && !((String)initValue).startsWith("'"))
					{
						initValue = null;
					}
				}
				...
				String type = null;
				if (initValue == null && (type = variable.getSerializableRuntimeProperty(IScriptProvider.TYPE)) != null)
				{
				...
				return new ProposalHolder(initValue, null, "Type: " + Column.getDisplayTypeString(variable.getVariableType()) + "<br/>Defaultvalue: " +
					variable.getDefaultValue() + "<br/>Form: " + ((Form)variable.getAncestor(IRepository.FORMS)).getName(), false, FORM_VARIABLE_IMAGE, file);
			}
	...
	public String[] getParameterNames()
	{
		...
		{
			String[] nms = new String[parameterNames.length];
			for (int i = 0; i < parameterNames.length; i++)
			{
				nms[i] = parameterNames[i];
			}
			return nms;
		}
		return null;
