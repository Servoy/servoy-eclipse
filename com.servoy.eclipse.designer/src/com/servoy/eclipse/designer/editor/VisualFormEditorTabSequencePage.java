...
public class VisualFormEditorTabSequencePage extends Composite
{
	...
	private IndexedListViewer selectedListViewer;
	...
	protected void createContents()
	{
		...
		selectedListViewer = new IndexedListViewer(container, SWT.BORDER | SWT.MULTI | SWT.H_SCROLL | SWT.V_SCROLL);
		selectedListViewer.addOpenListener(new IOpenListener()
		{
			public void open(OpenEvent event)
			{
				handleRemoveElements();
		...
		selectedListViewer.addSelectionChangedListener(new ISelectionChangedListener()
		{
			public void selectionChanged(SelectionChangedEvent event)
			{
				configureButtons();
		...
		selectedListViewer.setLabelProvider(LABELPROVIDER);
		selectedListViewer.setContentProvider(CONTENTPROVIDER);
		org.eclipse.swt.widgets.List selectedList = selectedListViewer.getList();
			...
			groupLayout.createSequentialGroup().addContainerGap().add(
				groupLayout.createParallelGroup(GroupLayout.LEADING).add(
					groupLayout.createSequentialGroup().add(
						...
						LayoutStyle.RELATED).add(
						groupLayout.createParallelGroup(GroupLayout.LEADING).add(selectedList, GroupLayout.PREFERRED_SIZE, 175, Short.MAX_VALUE).add(
		...
		groupLayout.setVerticalGroup(groupLayout.createParallelGroup(GroupLayout.LEADING).add(
			groupLayout.createSequentialGroup().addContainerGap().add(
				...
				groupLayout.createParallelGroup(GroupLayout.LEADING).add(
					...
					groupLayout.createParallelGroup(GroupLayout.LEADING).add(selectedList, GroupLayout.PREFERRED_SIZE, 188, Short.MAX_VALUE).add(
	...
	public void refresh()
	{
		...
		// editor is not we still refresh (setVisible() is not called so we might miss an update)
		{
			...
			ISelection selectedSelection = selectedListViewer.getSelection();
			...
			selectedListViewer.setInput(selected);

			availableListViewer.setSelection(availableSelection);
			selectedListViewer.setSelection(selectedSelection);
	...
	protected void configureButtons()
	{
		...
		IndexedStructuredSelection selection = (IndexedStructuredSelection)selectedListViewer.getSelection();
		boolean enableUp = false;
		boolean enableDown = false;
		if (selection.size() == 1)
		{
			int selectedIndex = selection.getSelectedIndices()[0];
			int nSelected = ((List<IPersist>)selectedListViewer.getInput()).size();
	...
	protected Command getSaveCommand(int[] tabSeqs, String label)
	{
		...
		List<ISupportTabSeq> selected = (List<ISupportTabSeq>)selectedListViewer.getInput();
	...
	protected void handleAddElements()
	{
		...
		executeCommand(new RefreshingCommand(getSaveCommand(tabSeqs, "add to tab sequence")) //$NON-NLS-1$
		{
			...
			public void refresh(boolean haveExecuted)
			{
				if (haveExecuted)
				{
					selectedListViewer.setSelection(selection);
				}
				else
				{
					// undo
	...
	{
		final IndexedStructuredSelection selection = ((IndexedStructuredSelection)selectedListViewer.getSelection());
	...
	protected void handleMoveElements(boolean up)
	{
		final IndexedStructuredSelection selection = ((IndexedStructuredSelection)selectedListViewer.getSelection());
		if (selection.size() == 1)
		{
			...
			{
				tabSeqs[nAvailable + selectedIndex - 1] = selectedIndex + 1;
				tabSeqs[nAvailable + selectedIndex] = selectedIndex;
			}
			else if (!up && selectedIndex < nSelected - 1)
			{
				tabSeqs[nAvailable + selectedIndex + 1] = selectedIndex + 1;
				tabSeqs[nAvailable + selectedIndex] = selectedIndex + 2;
			}

			executeCommand(new RefreshingCommand(getSaveCommand(tabSeqs, (up ? "de" : "in") + "crease tab index"))
			{
				@Override
				public void refresh(boolean haveExecuted)
				{
	...
	{
		int nFields = ((List<IPersist>)availableListViewer.getInput()).size() + ((List<IPersist>)selectedListViewer.getInput()).size();
