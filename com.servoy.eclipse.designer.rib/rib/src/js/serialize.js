...
$(function () {
     ...
     * @return {ADMNode/null} the design build from the text if success, null if failed.
     */
    function updateDesignToJSON(projectDesign, text, eachHandler) {
        var design, parsedObject, resultProject = {}, updateADMNode;

        updateADMNode = function (admNode, jsObject) {
            var item;
            ...
            try {
                ...
                if (admNode.isContainer()) {
                    ...
                    var children = jsObject.children || [];
                    
                   /*  ==== loop over zones =======*/
                    var zoneNames = BWidget.getZones(admNode.getType())
                    for (var z = 0; z < zoneNames.length; z++)
                    {
	                    var zoneName = zoneNames[z]
	                    
	                    var zoneChildren = admNode.getZoneArray(zoneName).slice() // clone
	                    var nodeIndex = 0
	                    ...
	                    for (var i = 0; i < children.length; i++)
	                    {
	                        ...
	                        if ((child.zone || "default") != zoneName) continue;
	                        
	                        var jsonChildId = null
	                        var childNode = null
	                        var n
	                        if (child.properties)
	                        {
	                        	jsonChildId = child.properties.id
	                        	for (n = nodeIndex; n < zoneChildren.length; n++)
	                        	{
	                        		if (zoneChildren[n].getProperty('id') == jsonChildId)
	                        		{
	                        			childNode = zoneChildren[n]
	                        			break
	                        		}
	                        	}
	                        }
	                        ...
	                        {
	                        	// no id, use next node
	                        	if (nodeIndex < zoneChildren.length)
	                        	{
	                        		childNode = zoneChildren[n = nodeIndex]
	                        	}
	                        }
	                        if (childNode != null && childNode.getType() != child.type)
	                        {
	                        	// node changed type, remove it and create a new one
	                        	admNode.removeChild(childNode, false);
	                        	childNode = null
	                        }
	                        if (childNode == null)
	                        {
	                        	childNode = ADM.createNode(child.type, true);
	                        	if (jsonChildId) childNode.setProperty('id', jsonChildId)
	                        	
	                        	// Add child node to current node
	                        	var zone = child.zone || "default";
	                        	if (!zonecounter[zone]) zonecounter[zone] = 0
	                        	if (!admNode.addChildToZone(childNode, zone, zonecounter[zone])) {
	                        		dumplog("add child type "+ child.type + " failed");
	                        		return false;
	                        	}
	                        }
	                        ...
	                        {
	                        	// delete extra child nodes
	                        	for (var j = nodeIndex; j < n; j++)
	                        	{
	                        		admNode.removeChild(zoneChildren[j], false);
	                        	}
	                        	nodeIndex = n+1
	                        }
	                        
	                        if (!zonecounter[childNode.getZone()]) zonecounter[childNode.getZone()] = 0
	                        zonecounter[childNode.getZone()]++
	                        
	                        if (!updateADMNode(childNode, child)) {
	                            return false;
	                        }
	                    }
	                    
	                    // delete extra child nodes
	                	for (var j = nodeIndex; j < zoneChildren.length; j++)
	                	{
	                		admNode.removeChild(zoneChildren[j], false);
	                	}
        ...
        // update children in ADM
        try {
            updateADMNode(projectDesign, parsedObject);
        } catch(e) {
            alert("Invalid design file.");
        }
    }
    ...
    function setDesignTheme(design, newTheme, newInSandbox) {
        ...
        if (newTheme === 'src/css/jquery.mobile.theme-1.3.1.css') {
